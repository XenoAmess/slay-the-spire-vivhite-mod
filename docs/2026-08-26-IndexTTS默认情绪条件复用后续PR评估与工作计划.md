...
- [ ] 等 #795 完成第一轮维护者 review，或合并到 upstream/main。
- [ ] 记录 #795 对公共参数命名、reference helper 和 WebUI 参数的最终意见。
- [ ] 在 #795 未定论前不提交 stacked follow-up，避免重复携带大段 diff。

### 阶段 2：独立分支实现

- [ ] 从更新后的 upstream/main 新建独立分支。
- [ ] 增加 `reuse_spk_cond_for_emo=False` 构造器参数和 public docstring。
- [ ] 仅在 2.5 WebUI 增加显式开关及帮助文本。
- [ ] 在隐式默认 emotion 分支接入条件 alias。
- [ ] 在同一特殊分支接入单次 `get_emovec()` fast path。
- [ ] 保持显式 emotion audio/vector/text 的原始路径不变。
- [ ] 不改变默认重采样链，不把此特化绑定到 CPU reference offload。

### 阶段 3：无 GPU 验证

按上游 [PR 模板](https://github.com/index-tts/index-tts/blob/main/.github/pull_request_template.md)准备 CPU/mock 测试；不启动本地模型、不占用直播显存：

- [ ] 默认关闭时，speaker 和 emotion helper 都被调用，行为保持旧路径。
- [ ] 开启且无显式 emotion source 时，emotion helper 不被调用，两个条件为同一对象。
- [ ] 开启时 `get_emovec()` 每个 segment 恰调用一次，`merge_emovec()` 不调用。
- [ ] 显式 emotion audio 生效时，即使路径与 speaker 相同，也仍独立编码。
- [ ] emotion vector / emotion text 路径不被错误短路。
- [ ] speaker 路径变化后不会继续使用旧 emotion alias。
- [ ] v2 传入该参数时得到明确错误。
- [ ] 运行 `uv run pytest -m "not gpu"`；如环境缺少依赖，至少完成 AST/语法和 mock 测试，并在 PR 中说明。

不额外购买或申请 GPU 测试。已有 GTX 1060 固定声线直播运行结果可作为真实使用记录，但不能包装成通用音质 A/B 证明。

### 阶段 4：PR 文案和审查应对

- [ ] PR 标题使用：`feat(inference): optionally reuse speaker conditioning for default emotion`。
- [ ] 正文明确写出：这是 opt-in 的计算/音频质量权衡，不是显存优化。
- [ ] 给出 `2 → 1` Wav2Vec reference forward 和每 segment `2 → 1` `get_emovec` 的结构性收益。
- [ ] 明确常驻显存收益约为 0；不声称缓存命中后 Wav2Vec 逐句提速。
- [ ] 明确 speaker/emotion 重采样链不同，结果不保证 bit-identical。
- [ ] 邀请有多种 GPU、语言和参考音频的社区成员补充音频 A/B。
- [ ] 如果维护者要求统一重采样，第一版不顺带做；统一链会扩大默认行为变化，应另行讨论。
...
如果 #795 被拒绝或维护者明确不接受“可能改变情绪条件的 opt-in”，本后续 PR 不继续推进；本地 wrapper 保留该特化即可。
