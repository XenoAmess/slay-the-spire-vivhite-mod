...
$candidateTres = Join-Path $ProjectDir "tools/candidates/hybrid_attack_peak/vivhite_combat_skeleton_data.tres"
if (-not [IO.File]::Exists($candidateTres)) {
    throw "Original Hybrid candidate .tres does not exist: $candidateTres"
}
$renderScript = Join-Path $PSScriptRoot "render_attack_peak_exact.gd"
if (-not [IO.File]::Exists($renderScript)) {
    throw "Exact renderer script does not exist: $renderScript"
}
...
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    ...
    $OutputDir = Join-Path $repoRoot ".work/combat-rig-compare-preview/hybrid-attack-peak-exact-$stamp"
}
...
try {
    ...
    Write-Host "[hybrid-attack-peak-exact] Rendering original candidate in a hidden off-screen Vulkan process..."
    ...
    if ($renderExit -ne 0) {
        throw "Hidden Vulkan Hybrid attack-peak acceptance failed with exit code $renderExit. See '$OutputDir'."
    }
    ...
    if ($summary.success -ne $true -or @($summary.frames).Count -ne 14 -or
        $summary.character_only_contract_passed -ne $true) {
        throw "Renderer report did not pass all 14 exact samples: $summaryPath"
    }
    foreach ($frame in @($summary.frames)) {
        if ($frame.composite.passed -ne $true -or $frame.character_only.passed -ne $true) {
            throw "Composite or character-only capture failed at t=$($frame.requested_time): $summaryPath"
        }
    }
    Write-Host "[hybrid-attack-peak-exact] Report: $summaryPath" -ForegroundColor Green
    Write-Host "[hybrid-attack-peak-exact] Composite sheet: $(Join-Path $OutputDir $summary.contact_sheets.composite)" -ForegroundColor Green
    Write-Host "[hybrid-attack-peak-exact] Character-only sheet: $(Join-Path $OutputDir $summary.contact_sheets.character_only)" -ForegroundColor Green
}
finally {
