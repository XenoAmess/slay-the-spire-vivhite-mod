...
- **维护 AI 是本项目的架构师，已启用的复盘模型是应由机制驱动、自行完成复盘的执行者。** 模型做错、卡住或未闭环时，优先定位并修复提示、证据、反馈、自检、观测、队列、提交或自愈机制，让下一次复盘能自己做对；不得长期由维护 AI 代写模型本应完成的具体策略成果来掩盖机制缺陷。维护 AI 可以审计、保全、回退和提供最小可复现用例，但最终目标始终是已启用模型可独立工作。
...
- 失败合入在机制问题修复后，必须由维护 AI 重新调用当前已启用的复盘模型，让模型自己重新审核失败批次、补合有效成果、解决与当前主树的冲突、运行自检并完成提交；维护 AI 负责提供完整失败现场、可读任务和验收/重试通道，不得默认亲自代做该批具体补合。只有模型重试机制本身仍有已证明故障时，维护 AI 才修该机制并再次触发模型。
...
- 新增运行时硬门禁还必须同时证明：无法复用已有校验、拒绝原因可观测、存在明确恢复路径、等待/重试有上界，并已同步覆盖旧夹具和端到端自检；任一项缺失时优先使用告警、观测或提交前校验，不得把普通异常升级成永久阻断。
