...
# 第 953~973 局复盘：升级轨迹下 STALL_ANY 闩锁提前一回合放行（HP_GATE_STALL_ESC_EARLY）

日期：2026-09-14

## HYPOTHESIS / EVIDENCE / EXPECTED_SIGNAL

- **HYPOTHESIS**：謦欬门拦空过死循环在敌意图递升轨迹下，STALL_ANY 闩锁
  （连续拦截≥4 + 敌血量零进展≥3）放行时点系统性偏晚——闩锁放行时「空过
  期间意图趋势累计」已普遍 ≥10，即每多空过这一回合，敌方意图净涨约 12
  点直接把战斗推深；在升级账已证明的死循环里提前一回合放行（连续拦截
  要求 4→3，零进展≥3 兜底不动），可减少一次纯浪费的空过回合与其战损，
  且不会向推进正常的战斗开门（零进展条件逐字保留）。
- **EVIDENCE**：本批 21/21 全负（生涯 1/973）。上批已登记本假设为「下一批
  头号行为化候选」（放行时点累计 929/935/946/949/952 五局≥阈值线）。本批
  程序化结算 927~973 全部 46 个 run 文件：HP_GATE_STALL_ESC_OBS 显形
  361 次；21 个独立（局，楼层）闩锁放行时点的「空过期间意图趋势累计」为
  [13,13,6,13,13,18,13,13,13,13,13,13,12,7,12,12,13,13,13,13,10]，
  18/21≥10（中位 12~13）——达成预登记行为化条件（普遍≥10 且该类战斗
  继续高战损：本批全负、放行局战损照旧自损反超）。973 局 F22 HUNTER_KILLER
  逐条核对：T3 门拦空过（尺度变换/黄金分割/终止条件实付4全拦、升级账
  +4），竞速 T3 判死→实战 6 回合阵亡；同一死循环若早一回合放行，T3 即可
  付血打出 18 伤尺度变换（IDLE_LEAK_RACE 审计同帧指认净缺口8）。
- **EXPECTED_SIGNAL**：未来 3~10 局 grep HP_GATE_STALL_ESC_EARLY：提前
  放行显形次数/局数、提前放行战斗的回合数与自损/掉血对照旧口径同型战斗
  （放行时点累计≥10 的死循环战）。提前放行战平均回合数下降或战损改善 →
  假设成立，维持并视证据微调阈值；显形战斗反而更快阵亡或放血回潮 →
  回滚=hp_gate_stall_esc_early=0（旧闩锁时点零差异，夹具③护住）。

## PRODUCTION_CHANGE

- sts2-ascend/brain/policy.py：__init__ 新增 _hp_gate_stall_early_fired
  （与 _hp_gate_stall_any_fired 同战斗实例生命周期，_combat_stall_check
  复位段连带清零）；STALL_ANY 闩锁判定前读取静态键
  hp_gate_stall_esc_early（默认 1）/hp_gate_stall_esc_early_intent
  （默认 12）——原阈值≥3 且升级账 _hp_gate_stall_esc≥阈值时，连续拦截
  要求减 1（下限 2）；放行条件其余逐字不动（零进展≥3、闩锁语义、评分/
  致死豁免不变）。仅当实际提前（放行时拦截数<原阈值）置位分流标记，
  放行注记追加「（升级轨迹提前放行：意图趋势累计+N，
  HP_GATE_STALL_ESC_EARLY）」。esc_early=0 一键回滚（旧行为零差异）；
  本段只在 _hp_play_margin>0 分支内，非白绮角色零改动。
- sts2-ascend/brain/knowledge.py：DEFAULT_POLICY 新增
  hp_gate_stall_esc_early=1、hp_gate_stall_esc_early_intent=12，注释
  登记本批实证与回滚语义。
- sts2-ascend/brain/selfcheck.py：新增 3pese 三分支——① 递升达阈值
  （5→12→19→26，升级账 19≥12）闩锁第 4 个 decide 提前放行、带 EARLY
  留痕、闩锁不回归、新战斗分流标记重置；② 递升低于阈值（3→5→7→9→11，
  升级账 7<12）维持原第 5 个 decide 放行且无 EARLY 留痕；③
  esc_early=0 严格回滚（同一递升序列闩锁时点逐字回旧口径、留痕全灭）。
  既有 3pesc 夹具显式钉住 hp_gate_stall_esc_early=0（单独验证观测注记
  的旧闩锁时点语义）。
- 不改余量门带、复打税、零压闸、自由回合减免与任何评分/演化路径；不动
  runs/stats/policy.json/lessons.md/review_queue 等只读在线状态。

## VALIDATION

- py -3 -B sts2-ascend/brain/selfcheck.py：SELFCHECK OK（新增 3pese 三
  分支；既有 3pesc 空过升级账族、3pri 僵局放行族、3prv 全拦截族、3prm
  未覆盖观测族、3krh/3krhm 判死自付族、3fdd/3fdl 换线阻尼/翻线锁族、
  3br 竞速投影族等全部既有夹具通过）。
- py -3 -B -m unittest sts2-ascend.tests.test_character_strategy：
  57 tests OK。
- py -3 -B -m unittest sts2-ascend.tests.test_review_decision_chain：
  10 tests OK。
- git diff --check -- sts2-ascend/ 通过；完整 diff 已回读：
  brain/policy.py（+37/-1，账字段+战斗复位+提前放行判定+放行注记）、
  brain/knowledge.py（+14/-1，两个静态键+注释）、brain/selfcheck.py
  （+87，3pese 三分支+3pesc 两处钉键）；未触碰只读在线状态；克隆残留的
  assets 超长路径删除告警为宿主挂载遗留，与本批无关、不入 commit。

## FOLLOW-UP / ROLLBACK

- 未来 3~10 局统计：HP_GATE_STALL_ESC_EARLY 显形次数与放行时点累计值
  分布（本批基线：21 个放行时点 18/21≥10、中位 12~13）；提前放行战斗的
  总回合数/自损/掉血，对照 927~973 同型（升级账≥10 放行战）旧口径。
- 提前放行战回合数下降或战损改善 ≥ 信号噪声 → 维持现值；显形战反而更快
  阵亡或放血回潮 → 回滚=hp_gate_stall_esc_early=0（旧闩锁时点零差异，
  夹具③护住）；阈值显著偏离（显形集中于累计≥18 或 7~11 频段大量错过）
  → 下一批微调 hp_gate_stall_esc_early_intent。
- 上批 INTANGIBLE_TTK_OBS 结算：本批 21 局窗口属其「未来 3~10 局」观察
  期中段，与主假设无冲突，下一批随竞速族夹具一并复核显形率。
...
# 第 1065~1086 局批复盘：竞速判死锁解观测（RACE_AUDIT_UNLATCH_OBS）——「判死后获胜」39.8% 混计口径的自修正/sticky 分流

日期：2026-09-15

## HYPOTHESIS

竞速判死入锁后实战获胜的战斗（stats.race_audit 台账 593/1490=39.8%，非 esc
桶 166/273=60.8%）主要由迟滞锁的自修正出口构成——投影在战斗中段翻回可行、
锁被解除后恢复正常攻防节奏获胜——而非 sticky 错标下全攻到底仍获胜。当前
`_race_audit` 只记入锁回合与 esc 桶，迟滞锁的两个合法出口（联合复核翻回
可行、全场无敌相）都不留痕，「判死后获胜」计数器把两种语义混计；而
RACE_AUDIT_HEAL_OVERRIDE（won/latched≥30% 保血）与
RACE_AUDIT_DOOM_WAIVER_GATE 消费的正是这个混计口径。该假设可证伪：未来
3~10 局战斗记录 grep RACE_AUDIT_UNLATCH_OBS——判死→获胜战斗大多带锁解段
→ 证实（锁标签在入锁时刻有效，获胜率主因是自修正，消费闸应按「终局仍武装
仍获胜」子集重校准）；大多无锁解段仍获胜 → 证伪（sticky 错标实锤，下一批
收紧锁）；锁解段长期零显形且获胜率不降 → 出口结构性不可达，同样指向收紧。

## EVIDENCE

- stats.race_audit 台账：latched=1490、won=593（39.8%）、died=897；
  esc_won=427/1217=35.1%，非 esc 桶 won=166/273=60.8%。非 esc 锁可被联合
  复核翻回、esc 锁被 RACE_ESC_LATCH_HOLD 锁持——非 esc 桶 60.8% 正是
  「自修正 vs sticky」语义分歧最大的桶。
- 本批 22 局全负：17 场判死入锁后阵亡、5 场判死后获胜（1067-F17 T2→T14
  胜、1071-F13 T5→T7 胜、1077-F6 T11→T11 胜、1078-F22 T2→T7 胜、1078-F29
  T8→T9 胜）；这 5 场获胜战中锁保持了多久、投影何时翻回，现有留痕（只有
  latch_round+结局）完全无法回答。
- 预注册口径（813~822 批）「判死→获胜 >30% → 行为化收紧」已被多处消费，
  但「锁自修正后获胜」是机制正常工作（不应收紧）、「sticky 错标仍获胜」
  才是标签错误（应收紧），两种语义被同一计数器混计。
- 迟滞锁清锁点全量盘点（grep `_krace_latch = False`）：战斗重置（新战斗
  实例，不计）、全场无敌相（policy.py 3222 附近）、联合复核翻回可行
  （3794 附近）——后两者为本批补记账的两个出口。
- 上一批（1052~1064，6a862545 RACE_AUDIT_POST_LATCH_PAY_OBS）发布 66 秒后
  被宿主安全撤销（3ffc36ad），未入 REVIEW_REJECTIONS、requested_packages
  为空；本批不重实现该包，落地语义独立、实现面更小的锁解观测。

## PRODUCTION_CHANGE

- sts2-ascend/brain/policy.py：
  - 新增 `_race_audit_note_unlatch(round_no, reason)` 纯观测助手：观测键
    开启且本场已入锁、尚未记过锁解时，把首次锁解的回合与原因记入
    `_race_audit`；锁的实际清除仍由调用点完成，未入锁/重复调用为 no-op，
    round_no 不可转 int 时静默放弃。评分/判决/动作零改动。
  - 两个锁解出口接线：全场无敌相清除前记 `invuln`；联合复核翻回可行、
    确曾持锁时记 `flip`（RACE_ESC_LATCH_HOLD 锁持分支不经过此出口，
    语义不变）。
  - `pop_race_audit`：仅当快照含 `unlatched_round` 时附 `ended_latched`
    （终局锁态，区分锁解后再入锁的边缘形）；普通入锁战斗快照严格保持旧
    键集（既有精确相等夹具锚不动）。
- sts2-ascend/brain/knowledge.py：DEFAULT_POLICY 新增静态键
  `race_audit_unlatch_obs=True`（含台账证据注释；False=停止记账与披露，
  旧行为零差异）。
- sts2-ascend/brain/agent.py：战斗记录竞速审计段在含锁解键时追加
  「锁解T<回合>（<原因>，<终局锁态>，RACE_AUDIT_UNLATCH_OBS）」；无锁解
  键时段落严格回落旧口径，审计段与（阵亡）后缀位置不变。
- sts2-ascend/brain/selfcheck.py：3rau 四段（① 入锁+锁解+弹出快照精确
  键集；② 重复锁解不覆盖首次且 ended_latched 反映终局锁态；③ 键关后
  零记账、弹出严格旧键集回滚锚；④ 未入锁 no-op）+ 3rau-b 三段（锁解
  披露文本与（阵亡）后缀位置、终局仍武装形、无锁解严格回落旧口径）。
- 未触碰竞速判决/门带/评分/任何 stats 并表口径；runs/stats/policy.json/
  lessons.md/review_queue 等只读在线状态未动。

## VALIDATION

- py -3 -B sts2-ascend/brain/selfcheck.py：SELFCHECK OK（含 3rau 四段与
  3rau-b 三段；既有 pop_race_audit 精确相等夹具、3y 审计并表、3krh/
  3krhm/3krds 判死自付族、3pese/3pesc 提前放行族全量通过）。
- py -3 -B -m unittest sts2-ascend.tests.test_character_strategy
  sts2-ascend.tests.test_review_decision_chain：67 tests OK。
- git diff --check 通过；完整 diff 已回读：brain/policy.py（+43/-1，助手+
  两出口+pop 终局锁态）、brain/knowledge.py（+15/-3，静态键+证据注释）、
  brain/agent.py（+15，披露段）、brain/selfcheck.py（+66，3rau/3rau-b
  夹具）；未触碰在线状态；工作区 assets/ 长路径删除告警为宿主预置现场，
  与本批无关、不入 commit。随后本地 commit。

## FOLLOW-UP / ROLLBACK

- 未来 3~10 局 grep RACE_AUDIT_UNLATCH_OBS：统计判死→获胜战斗的锁解段
  占比（预期非 esc 桶多数带段）、「终局仍武装+获胜」子集计数（真 sticky
  错标样本）、flip/invuln 出口分布。
- 判死→获胜大多带锁解段 → 下一批把 HEAL_OVERRIDE/入场线豁免审计闸的
  获胜率口径改计「终局仍武装仍获胜」子集（本批不动闸）；大多无锁解段
  仍获胜 → 下一批收紧锁（入锁门槛/HOLD 闸/锁再评估）；锁解段长期零
  显形且判死→获胜率不降 → 出口结构性不可达，同样进入收紧评审。
- 撤回条件：审计段解析报错、竞速判决或评分出现任何偏移（本改动零评分
  路径，理论上不可能）→ `race_audit_unlatch_obs=False` 一键撤回，记账
  与披露同步消失，旧行为零差异（3rau③/3rau-b③ 锚住）。
