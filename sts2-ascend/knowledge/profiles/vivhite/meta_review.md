...
# 第 615~619 局批复盘：謦欬 1HP 全手锁死终局显形——锁死只活在决策长文本、战斗记录无计数（VIVHITE_HP_LOCKOUT_OBS 观测位）

日期：2026-09-11

## HYPOTHESIS / EVIDENCE / EXPECTED_SIGNAL

- **HYPOTHESIS**：謦欬 1HP 全场锁死（全部非诅咒手牌 blocked_by_hook，任何
  出牌都会令生命低于 1）是终局战的致死放大器——自付压到 1 血后下回合
  整手锁死、攻防双停；但该状态目前只在决策 reason 长文本留痕，
  combat_notes/stats 无计数，复盘无法量化其频率、出现楼层与致死关联，
  也无法判断生命支付端是否需要「锁死保留线」（非致死回合拒绝自付到
  1 血）行为化。
- **EVIDENCE**：本批 5 局中 4 局终局战出现锁死收口——615-F33（1 次）、
  616-F17（1 次）、618-F45（1 次）、619-F17（2 次）。619 局
  （MR8F62NQL7YL）完整链逐条核对：F17 Boss 战（SOUL_FYSH）T8 我方 2 血
  打出开集庇护实付 1 血→1 血（当回合意图 10、格挡 14 属实保命），随后
  T9/T10「当前1生命，全部非诅咒手牌因謦欬会令生命低于1」整手锁死，
  T10 意图 13 零格挡阵亡（掉血61｜自损27）。同批謦欬门体系（零压闸/
  攻击豁免/余量门/僵局闩锁）在普通战工作正常，泄漏扫描（5 局全部
  COMBAT 出牌决策×意图0×hp-cost>0）未见零压闸新漏洞：Monster 房意图0
  付血全部带 VIVHITE_HP_ZP_ATTACK_EXEMPT 豁免注记或击杀豁免，Unknown
  房本批无意图0付血样本——锁死观测是唯一无承接的新形态。
- **EXPECTED_SIGNAL**：未来 3~10 局——① combat_notes 出现
  「謦欬锁死N回合（VIVHITE_HP_LOCKOUT_OBS）」段，可直接计数而无需回放
  决策链；② 若锁死回合集中出现在终局战且与阵亡强相关（本批 4/5 即如此）
  → 下一批评估「锁死保留线」行为化（非致死/非保命回合拒绝自付到 1 血）；
  ③ 若锁死实际罕见或集中于数学必败局 → 关闭观测键即可，不追加行为。
  证伪/撤回：观测段格式漂移或误计入非锁死回合 → policy.json 置
  vivhite_hp_lockout_obs=false 一键回落旧战斗记录口径（旧行为零差异）。

## PRODUCTION_CHANGE

- sts2-ascend/brain/policy.py：
  - `Policy.__init__` 新增 `_tl_lock_rounds` 计数；主 `_combat` 的
    ctx.combat 实例更替重置块同步清零（多阶段 Boss 每阶段 `_start_combat`
    新建 ctx.combat，分段口径不重复累计）；
  - 锁死收口分支（确认两拍后 end_turn，动作与旧口径严格一致）计数 +1；
  - 新增只读接口 `combat_terminal_life_lock_rounds()`（口径同
    combat_self_hp_loss，按战斗实例隔离）。
- sts2-ascend/brain/agent.py：`_settle_combat` 按分段读取锁死计数并入
  聚合账 `tl_lock_rounds`；`_flush_combat_agg` 在自损段之后、竞速审计与
  （阵亡）后缀之前披露「｜謦欬锁死N回合（VIVHITE_HP_LOCKOUT_OBS）」，
  观测键 `vivhite_hp_lockout_obs`（默认 True）关闭时段落严格回落旧口径。
- sts2-ascend/brain/knowledge.py：DEFAULT_POLICY 新增静态键
  `vivhite_hp_lockout_obs: True`（注释记录跨局实证与回滚口径）。
- sts2-ascend/brain/selfcheck.py：新增 3tlk 四锚——① 619-F17 T10 复刻
  载荷（1 血/整手 LifeCost=2/无 play_card）确认两拍后收口 end_turn 且
  计数+1（确认等待拍不计入）；② ctx.combat 实例更替后计数重置；
  ③ 战斗记录披露段可 grep 且（阵亡）后缀位置不变；③b 观测键=False
  严格回落旧口径；④ 零锁死战斗不出披露段。
- 纯观测改动：不改任何评分、阈值、门拦或动作选择；旧记录（无
  tl_lock_rounds 键）按 0 处理零差异；非白绮角色锁死分支不触发、
  披露段恒不出现的双重零改动。

## VALIDATION

- py -3 -B sts2-ascend/brain/selfcheck.py：SELFCHECK OK（3tlk 四锚新增；
  既有 3slph 五锚、3prz 零压闸六锚、3prn/3prh/3pri/3prm/3prt/3prg/3prl、
  3ps、3sg、3br-5/3br-6、3br-act-trunc、3kd、3sec 等全部既有夹具通过）。
- py -3 -B -m unittest sts2-ascend.tests.test_character_strategy：57 tests OK。
- git diff --check -- sts2-ascend/ 通过；完整 diff 已回读：policy.py（+25，
  计数+重置+只读接口）、agent.py（+23/-1，分段并入+披露段）、
  knowledge.py（+13 一个静态键）、selfcheck.py（+89，3tlk 四锚）+
  本报告与口播短评；未触碰 runs/stats/policy.json/lessons.md/review_queue
  等只读在线状态；克隆残留的 assets 超长路径删除告警为宿主挂载遗留，
  与本批无关、不入 commit。
