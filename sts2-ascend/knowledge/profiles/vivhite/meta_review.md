...
# 第 1181~1184 局批复盘：多强化体重复换线的自付/生命状态观测

日期：2026-09-16

## HYPOTHESIS / EVIDENCE / EXPECTED_SIGNAL

- **HYPOTHESIS**：当至少两名仍持有力量层数的敌人同时存活时，第二次及以后非击杀实际换线
  可能是「持续自付、进入濒死窗口」的风险标记，而不只是普通火线漂移。若该假设成立，
  触发时的累计隔离自付血应偏高、当前生命应偏低，并在同局与
  `SELF_LOSS_PHASE_OBS`、判死后的实际存活回合或终局阵亡形成可重复对账；若未来至少
  3 个独立触发局中这些状态不比普通换线更接近自损/濒死，或与结果没有关系，则假设证伪。
- **EVIDENCE**：本批完整最新局为 1184（F33，`runs/20260916-152725_M0WZC2B3L6PP.json`），
  CRUSHER/ROCKET 同场均有力量层数；第 2 次多强化体非击杀换线在当前生命 7、已隔离累计
  自付 36 血时触发，随后本局自损 39、掉血 94，T2 竞速判死后实战第 5 回合阵亡。
  全局还记录 `FOCUS_DRIFT_OBS=4`、`FOCUS_DRIFT_LOCK=8`、
  `RACE_SAME_ROUND_HP_LOSS_OBS=36`。原生 `monsters.jsonl` 也确认两种敌人各自拥有
  `AdaptStrengthGain`/`ChargeUpStrengthGain` 强化动作；1180-F35 已有一次同形态证据，
  目前为 2 个独立触发局，尚未达到 `evidence_run_threshold=3`。
- **EXPECTED_SIGNAL**：未来 3~10 局统计
  `FOCUS_DRIFT_MULTI_SCALER_OBS` 与其状态字段的触发次数/局，读取每次触发的累计自付、
  当前生命，并与同局 `SELF_LOSS_PHASE_OBS`、`FOCUS_DRIFT_LOCK`、判死后实际存活回合
  对照；没有触发属于场景缺席，不单独判假设成立。

## PRODUCTION_CHANGE

- `sts2-ascend/brain/knowledge.py` 新增默认开启的
  `focus_drift_multi_scaler_state_obs`。在既有
  `FOCUS_DRIFT_MULTI_SCALER_OBS` 首次命中时，`sts2-ascend/brain/policy.py` 仅向出牌
  `why` 追加 `FOCUS_DRIFT_MULTI_SCALER_STATE_OBS`，记录触发瞬间累计自付血与当前生命。
  该字段不进入评分、目标选择、放行闸或动作选择；没有行为调参。
- `sts2-ascend/brain/selfcheck.py` 的 3fdm 覆盖默认值、触发值、战斗内去重，以及关闭新键
  后只移除数值字段而保留既有定性标记；稳定 combat fixture 只用于避免测试台账被重置。

## VALIDATION

- 宿主默认自检入口在第 256 个临时目录时触发既有 bootstrap pool exhaustion；未改动该引导器。
  使用等价的 `py -3 -B -S` 内存临时目录包装器执行，结果为 `SELFCHECK OK`。
- 同一包装器下 `test_character_strategy`：57/57 OK；`test_review_decision_chain`：10/10 OK。
- `git diff --check` 退出码 0；输出的 `assets/` 长路径提示来自现有现场，未纳入本次提交。

## FOLLOW-UP / ROLLBACK

- 保持 `focus_drift` 阻尼/锁定与所有评分路径不变，先累计满 3 个独立触发局；逐局核对
  新状态与自付、生命、实际存活和终局结果。若状态字段解析错误、触发条件与两名力量体
  不一致，或出现可观测行为漂移，将 `focus_drift_multi_scaler_state_obs=False` 回滚；
  若既有定性标记本身异常，再独立回滚 `focus_drift_multi_scaler_obs=False`。
