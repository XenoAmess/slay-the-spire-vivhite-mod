...
# 第 697~701 局批复盘：知识恶魔 Ponder 回血对斩杀竞速投影完全失明——ttk 净口径（KNOWLEDGE_DEMON_PONDER_HEAL）

日期：2026-09-11

## HYPOTHESIS / EVIDENCE / EXPECTED_SIGNAL

- **HYPOTHESIS**：知识恶魔原生 PonderMove（v0.111.0 mechanics/monsters.jsonl
  实证）每 4 回合执行一次 CreatureCmd.Heal(30×players) 并 +2 力量，行动循环
  CURSE→SLAP→OVERWHELMING→PONDER（诅咒计数≥3 后缩为 3 回合更频）。斩杀竞速
  投影 ttk=血池/实测 dpt 对敌方回血完全失明——存活侧意图逐 tick 观测已含
  Ponder 力量复利，唯独击杀侧分子从不计入回血量，ttk 被系统性低估，判死与
  全攻换挡偏晚，贴线局被「假可行」拖进磨死。给 ttk 改净口径
  （net_dpt=dpt−7.5/回合，30/4 保守档）后投影反映真实击杀时间。可证伪：
  未来 3~10 局知识恶魔战决策链若不出现 KNOWLEDGE_DEMON_PONDER_HEAL 注记
  （enemy_id/name 双通道接线或载荷不可读），或注记显形但实战 Ponder 从未
  回血（机制理解错误），或更早判死把贴线可赢局推向放弃（竞速审计
  won/latched 较 47.6% 基线显著恶化），则本假设不成立。
- **EVIDENCE**：701 局（V8D6QSFBP3MV，F33 知识恶魔，掉血103 阵亡）全链逐
  tick 核对——敌方意图序列 0/17/24/11/0/19/30/13/0/21 与原生循环严格对齐：
  T4（意图11）与 T8（意图13）为两次 Ponder，各回 30 血共 60，等效血池
  379+60=439 而投影只按字面 379 计价，ttk 低估 ≈14%+；T6 投影仍按「敌血池
  118」对账，未含 T8 即将到来的 +30。KNOWLEDGE_DEMON 以 36.2 权重居生涯
  deaths_by_enemy 首位（次位 THE_INSATIABLE 28.2）；同型磨死还有 595 局
  F33（Boss 余 ~16 血阵亡）与 488 局 F33。原生机制核对（v0.111.0
  mechanics/monsters.jsonl）：PonderMove=DamageCmd.Attack(11)+
  CreatureCmd.Heal(30)+PowerCmd.Apply<StrengthPower>(2)；
  GenerateMoveStateMachine 证实 4 态循环与诅咒计数分支。
- **EXPECTED_SIGNAL**：未来 3~10 局——① 知识恶魔存活且旋钮>0 的战斗决策链
  出现「知识恶魔Ponder回血7.5/回合入净口径：净输出X→Y伤/回合，ttkA→B
  回合（KNOWLEDGE_DEMON_PONDER_HEAL）」注记（B>A）；② 同型局判死时点不
  晚于旧裸口径，贴线局「实战获胜但投影判死/判死但实战磨死」对账偏差缩小；
  ③ 非知识恶魔战斗零注记、ttk 与判决逐位同旧。证伪/撤回：注记零显形 →
  复查 enemies 载荷 enemy_id/name 双通道；注记显形但实战零回血 → 重新核对
  Ascension/回合循环假设；贴线可赢局被误压 → policy.json 置
  knowledge_demon_ponder_heal_rate=0 一键回滚（净口径段整体旁路，ttk 逐位
  回旧、注记消失，零残留）。

## PRODUCTION_CHANGE

- sts2-ascend/brain/policy.py：_combat_kill_race_projection 内 ttk 计算点
  改为净口径——任一存活敌人 enemy_id==KNOWLEDGE_DEMON 或 name==知识恶魔
  （双通道，与既有敌型识别同构）时 net_dpt=max(1,dpt−heal_rate)，
  ttk=pool/net_dpt 并追加 KNOWLEDGE_DEMON_PONDER_HEAL 对账注记（含
  raw→net ttk 读数）；旋钮缺失默认 7.5、=0 走 else 分支严格回滚旧公式。
  tsurv、loss_rate、防守线联合复核、迟滞锁、滑溜/沙坑/无敌帧全部既有体系
  不动；非知识恶魔敌人逐位零差异。
- sts2-ascend/brain/knowledge.py：DEFAULT_POLICY 新增静态键
  knowledge_demon_ponder_heal_rate: 7.5，注释记录原生机制出处、701 局实证
  与回滚口径（0=关闭，严格回滚裸口径）。
- sts2-ascend/brain/selfcheck.py：新增 3kdh 三分支——① 知识恶魔在场净口径
  ttk9→13 翻转判死且注记显形；② 键=0 严格回滚（无注记、不判死）；③ 同
  参数非知识恶魔敌人无注记不判死（机制隔离）。
- 不改敌型识别任何既有函数、不改 dpt/tsurv 任何既有口径；不动
  runs/stats/policy.json/lessons.md/review_queue 等只读在线状态。

## VALIDATION

- py -3 -B sts2-ascend/brain/selfcheck.py：SELFCHECK OK（新增 3kdh 三分支；
  既有 3sec 沙坑封底族、3br 滑溜族、3sg 沉睡守卫族、3et 激怒税族、3kd
  诅咒税族等全部既有夹具通过）。
- py -3 -B -m unittest sts2-ascend.tests.test_character_strategy：57 tests OK。
- git diff --check -- sts2-ascend/ 通过；完整 diff 已回读：brain/policy.py
  （+36，ttk 净口径一段）、brain/knowledge.py（+13 一个静态键及注释）、
  brain/selfcheck.py（+79，3kdh 夹具）；未触碰只读在线状态；克隆残留的
  assets 超长路径删除告警为宿主挂载遗留，与本批无关、不入 commit。
