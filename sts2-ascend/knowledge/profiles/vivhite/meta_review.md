...
# 第 807~812 局批复盘：竞速判死时点误差零台账——入锁 gap 分档观测重落地（RACE_AUDIT_LATCH_GAP）
...
- **HYPOTHESIS**：竞速判死入锁的「时点误差」（判死回合→实战终结回合的
  ...
  （RACE_AUDIT_STATS_AGGREGATION）只有 latched/won/died/esc_* 胜负计数，
  没有时间维度——每批复盘只能逐局 grep combat note 的「T3判死→实战11
  回合」字符串。第 790~806 批已落地同一 gap 分档观测（9011dcf3），但在
  产出任何数据前被 runner 以「新代码启动失败」安全撤销（9fcdeb26）——
  该测量从未上线。本批已在隔离 clone 内逐字节复核被撤销代码：AST 解析
  通过、完整 selfcheck 通过（含被改动的 3y 夹具），判定代码本身无缺陷，
  启动失败属环境性；基于当前 HEAD 重实现同一语义观测。
- **EVIDENCE**：本批 6 局全负（生涯 1/812），8 场入锁战斗逐局核对：807
  T5→T6(gap1，阵亡)、808 T5→T6(gap1，获胜)/T5→T8(gap3，阵亡)、809
  T2→T5(gap3)、810 T2→T5(gap3)、811 T2→T7(gap5)、812-F27 T6→T7(gap1，
  获胜)/812-F33 T3→T11(gap8)——died gap 分布 1/3/3/3/5/8（均值 3.8），
  6 场阵亡中 4 场实战比判死多活 ≥3 回合；与 790~806 批 17 场 died gap
  0~8（均值约 2.9）同向。台账现状 latched=1107/won=444（40.1%），仍超
  30% 预注册线但无法回答「早多少」。812 局（2FPX7RX6TZW4，F33 知识恶魔）
  111 条保留决策链已逐条复核：T3 判死后实战 11 回合、输出 33~37/回合
  高于预演口径，换挡时点误差数据缺口与假设方向一致。
- **EXPECTED_SIGNAL**：未来 3~10 局 stats.race_audit 新增
  won_gap_*/died_gap_*（le1/2_3/4_6/ge7 档 + sum/n），随 stats digest
  直接进入后续 packet，无需再 grep。预注册消费规则：died_gap 均值 >2
  且 4_6+ge7 档占比过半 → 下一批行为化收紧换挡时点；died_gap 均值
  ≤1.5 → 判死时点准确，冻结该方向不再投入。证伪/撤回：本段为纯观测
  （无评分/阈值分支），撤回=删除 agent.py 本段并恢复 selfcheck 3y
  旧期望，零行为差异。

## PRODUCTION_CHANGE

- sts2-ascend/brain/agent.py：_flush_combat_agg 的
  RACE_AUDIT_STATS_AGGREGATION 块追加 gap=rounds-latch_round 分档累计
  （won/died 分前缀，le1/2_3/4_6/ge7 + sum/n；latch_round 缺失或
  rounds≤0 时跳过；TypeError/ValueError 防御置 None）。不参与任何
  评分/阈值分支，combat note 字符串与「（阵亡）」后缀位置不变。
- sts2-ascend/brain/selfcheck.py：3y 夹具更新——won gap5→won_gap_4_6、
  died gap6→died_gap_4_6；新增 latch1→gap7=died_gap_ge7 与
  latch8→gap0=died_gap_le1 边界用例（sum=13.0/n=3）；未入锁战斗仍零改动。
- 未触碰 runs/stats/policy.json/lessons.md/review_queue 等只读在线状态。

## VALIDATION

- py -3 -B sts2-ascend/brain/selfcheck.py：SELFCHECK OK（3y 夹具全分支
  通过；既有滑溜族、謦欬门族、3tr 荆棘族、3fdo 火线漂移族等全部既有
  夹具通过）。
- py -3 -B -m unittest sts2-ascend.tests.test_character_strategy：
  57 tests OK。
- py -3 -B -m unittest sts2-ascend.tests.test_review_decision_chain：
  10 tests OK。
- 被撤销版（9011dcf3）agent.py+selfcheck.py 在本 clone 临时树复核：
  AST 解析通过、selfcheck OK，证明上次安全撤销非代码缺陷。
- git diff --check -- sts2-ascend/ 通过；完整 diff 已回读：agent.py +30
  （观测段+注释）、selfcheck.py +33/-5（3y 夹具），无意外文件；
  assets/ 下既有删除为克隆挂载现场，与本批无关、不纳入 commit。

## FOLLOW-UP / ROLLBACK

- 未来 3~10 局从 packet stats_digest.race_audit 直接读 died_gap 分布，
  按上方预注册规则决定换挡时点是否行为化收紧；won_gap 持续 ≥3 也佐证
  「判死后仍可磨下」，与现有 40% 胜率闸互相印证。
- 若再次因「新代码启动失败」被 runner 安全撤销：代码已两次验证无缺陷，
  宿主应排查启动验证环境（锁/预算/API 就绪），而非继续回滚本段。
- 回滚=删除 agent.py 观测段并恢复 selfcheck 3y 旧期望（零行为差异）。
