...
# 第 392~401 局批复盘：审计回血纠错带顶 0.65 贴线拒救（65.7% 差 0.7pp 被弃疗）——贴线扩展顶 ext 只放宽不收紧

日期：2026-09-09

## HYPOTHESIS

竞速审计回血纠错带（RACE_AUDIT_HEAL_OVERRIDE）的血量上限 0.65 在判死台账 41.5% 假阳性体制下仍过窄：第 392 局前夜 65/99=65.7% 以 0.7pp 之差错过带顶，被「必败弃疗改锻造」抢走回血；实战 F35 知识恶魔 11 回合阵亡、掉血 65 整管打空（自损 36+敌方净伤 29）——+29.7 回血（约 95 进场）即生还约 30 点。给纠错带加贴线扩展顶（max(原上限, 0.75)）后，65%~75% 贴线前夜在审计门槛成立时重新获得回血。

该假设可证伪：未来 3~10 局若 65%~75% 贴线前夜的 RACE_AUDIT_HEAL_OVERRIDE 留痕出现但被放行局的 Boss 生还率/战损无改善，或 >75% 高血前夜被误触发覆盖，则假设不成立，policy.json 置 `boss_eve_race_audit_heal_hp_cap_ext=0` 一键回滚（旧上限行为零差异）。

## EVIDENCE

- 失败包 20260909-003813-1788885493026293500-e77b9c21 完整 lineage 已读：manifest/inventory/report/wip.patch/retry_candidate.patch 全部可读；失败原因是 patch 触碰 deny-only 边界 `tmp_392.txt`（Git/outside/unsafe），候选改动本身（knowledge.py/policy.py/selfcheck.py 三个生产文件）经 inventory 认证为 accepted_candidate_paths。本批在当前 HEAD 自行重实现该有效部分，剔除越界文件，并按当前证据校正注释数字。
- 第 392 局（C3PLQQGAUG1S）决策 447（F34 篝火）逐条核对：当前 66%（65/99=65.7%），「必败弃疗改锻造」，竞速预演判击杀需 12 回合＞满血可存活 7 回合；决策 451~493（F35 知识恶魔）逐条核对：65 血 0 甲进场，T11 以 5 血中 36 意图阵亡；战斗记录「F35 Boss战 掉血65｜自损36」——总损失恰等于入场血量，+29.7 回血即生还约 30。
- 台账与批内对照：stats.race_audit=238/573=41.5%≥30% 预注册线；本批 10 局战斗记录竞速判死 23 次、实战 16 胜 7 死（70%）；批内前夜裁决扫描：392-F34 SMITH 66%（本批目标）、396-F16 AUDIT_HEAL 36%、393-F47 HEAL 60%、400 两局 HEAL 84%/87%——65%~75% 贴线带正是唯一被拒救区间。
- 最新死亡局 run 401（DFU9AZHBLKK1，F33 知识恶魔阵亡）完整链切片（30 条保留决策+聚合表，complete_persisted_chain=false）已逐条检查：与本假设无冲突，F1 路径规划留痕可见审计闸已生效（RACE_AUDIT_DOOM_WAIVER_GATE 否决豁免）。

## PRODUCTION_CHANGE

- sts2-ascend/brain/knowledge.py：DEFAULT_POLICY 新增静态键 `boss_eve_race_audit_heal_hp_cap_ext=0.75`（0 严格复原旧上限）。
- sts2-ascend/brain/policy.py：`_boss_eve_race_audit_heal` 的 audit_cap 在原 min(专用键, 证据封顶) 之后取 `max(audit_cap, clamp(ext,0,1))`——只放宽不收紧既有上限；ext=0/缺省严格旧行为；地图投影经同一函数自动镜像。
- sts2-ascend/brain/selfcheck.py：3br-audit-cap 夹具接入 ext（hp_cap 复原段同时置 ext=0）；新增 3br-audit-cap-ext 三分支——54/80=67.5% 贴线带触发审计回血并带留痕、ext=0 严格复原不触发、高血对照上调至 62/80=77.5% 仍不触发。
- 不改评分/阈值/动作选择主体；不动 runs/stats/policy.json/lessons.md/review_queue 等只读在线状态。

## EXPECTED_SIGNAL

未来 3~10 局：① 65%~75% 贴线判死前夜出现 RACE_AUDIT_HEAL_OVERRIDE 回血留痕（可与同决策「当前 N%」对账）；② 被放行局的 Boss 入场血量上移、F17/F33/F35 型整管打空案例减少；③ race_audit won/latched 不恶化。证伪/回滚条件：留痕 ≥3 局但生还率/战损无变化，或 >75% 高血前夜误触发——policy.json 置 `boss_eve_race_audit_heal_hp_cap_ext=0` 整体撤回。

## VALIDATION

- `py -3 -B sts2-ascend/brain/selfcheck.py`：SELFCHECK OK（含修订后的 3br-audit-cap 与新增 3br-audit-cap-ext，全部既有夹具通过）。
- 完整 diff 已回读：brain/knowledge.py（+7 一个静态键）、brain/policy.py（+11 带顶扩展）、brain/selfcheck.py（+23/-2 夹具）三个生产/测试文件 + 本报告与口播短评；未触碰 runs/stats/policy.json/lessons.md/review_queue 等只读在线状态；克隆残留的 assets 超长路径删除告警为宿主挂载遗留，与本批无关、不入 commit。

## REPLAY

retry_resolution: 20260909-003813-1788885493026293500-e77b9c21 integrated（失败原因为 patch 触碰 deny-only 路径 tmp_392.txt；候选三文件生产改动已在当前 HEAD 重实现并自检通过，越界文件剔除，证据数字按当前 HEAD 校正为 238/573=41.5% 与掉血 65=自损 36+敌净 29）
