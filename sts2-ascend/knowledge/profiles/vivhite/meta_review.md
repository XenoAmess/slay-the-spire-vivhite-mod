...
# 第 1052~1064 局批复盘：锁后自付分账观测（RACE_AUDIT_POST_LATCH_PAY_OBS）——doomed window 自付集中度首次可跨局对账

日期：2026-09-15

## HYPOTHESIS

白绮在竞速判死入锁后的 doomed window（入锁回合→战斗终结）内仍集中实付生命，
足以烧穿可存活分母、自证死期；但战斗记录的「竞速审计」段只有入锁回合与实战
结局，锁后自付没有聚合口径，该假设此前无法用生产留痕直接证实或证伪。

## EVIDENCE

- 1064 局 0J7DVZE0NQ9A（runs/20260915-131014_0J7DVZE0NQ9A.json）F33
  CRUSHER+ROCKET 战完整决策链：T1 末 JOINT_FLIP_TTK_CAP 否决防守线翻盘
  （击杀需 10 回合＞1.5×可存活 5 回合），T4 实测口径判死入锁；入锁后
  13:24:04~13:24:23 六张牌带 KILL_RACE_HOPELESS_HP_PAY_OBS 自付
  2+2+6+4+4+2=20 血，占全场可行动段自损 28 的 71%；终段 2 血全部手牌
  blocked_by_hook，实战 T7 阵亡（竞速审计：T4判死→实战7回合阵亡）。
- 本批 13 局全负，12 场战斗带「判死→实战」对照：1053 T6→7亡、1054 T2→8亡、
  1056 T3→9亡、1058 T4→8亡、1059 F17 T2→16胜/F22 T3→8亡、1060 T2→7亡、
  1061 F17 T2→7胜/F31 T3→7亡、1062 T2→7亡、1063 T2→8亡、1064 T4→7亡，
  另有 1057 两场翻盘获胜——8 场阵亡的 doomed window 长达 3~6 回合，4 场
  翻盘证明锁后自付既可能是纯放血也可能是翻盘燃料，正是需要逐场切片的层。
- 现有账本均无法切这层：KILL_RACE_HOPELESS_HP_PAY_OBS 只按出牌逐条注记
  （跨局要回放决策链手工点数）、SELF_LOSS_PHASE_OBS 只有全场可行动/非行动
  相位分账、stats.race_audit 只有胜负计数——没有任何「入锁后自付」聚合
  口径，上批 DOM_SCALE 行为化的成效也因此无法按锁后窗口验收。

## PRODUCTION_CHANGE

- sts2-ascend/brain/policy.py：
  - 战斗重置块新增 `_race_self_paid_by_round` 逐回合自付影子账（跨战斗清零）。
  - `_combat` 回合边界在 `_self_paid_round_obs` 算出后，观测键开启且自付>0 时
    把可行动段自付累进影子账（键=刚结束的回合号）；评分/放行/动作零改动。
  - `pop_race_audit` 弹出 latched 审计账时按入锁回合切分锁前/锁后两桶
    （paid_before_latch / paid_after_latch）；未完结当前回合用相位影子分账
    残差补记（该回合必无旧账，无双计）；观测键关闭时不附分账键。
- sts2-ascend/brain/knowledge.py：DEFAULT_POLICY 新增静态键
  `race_audit_post_latch_pay_obs=True`（含证据注释；False=停止记账与披露，
  旧行为零差异）。
- sts2-ascend/brain/agent.py：战斗记录竞速审计段在分账键存在时追加
  「锁后自付X（RACE_AUDIT_POST_LATCH_PAY_OBS）」（口径：入锁当回合计入锁
  后，为上界）；审计段与（阵亡）后缀位置不变，零自付战斗不附误导性 0。
- sts2-ascend/brain/selfcheck.py：3plp) 四段——① 三段时序逐回合账
  {1:8,2:6}+T3 残差 6，入锁 T2 弹出切分 锁后12/锁前8；② 回滚锚ⓐ 键关
  后不记账、不附分账键；③ 战斗记录披露「T4判死→实战7回合阵亡，锁后自付
  20（RACE_AUDIT_POST_LATCH_PAY_OBS）」且（阵亡）后缀位置不变；④ 回滚
  锚ⓑ 键关后审计段严格回落旧口径。
- 未触碰评分/门带/竞速判决/任何在线状态；runs/stats/policy.json/lessons.md/
  review_queue 等只读在线状态未动。

## VALIDATION

- py -3 -B sts2-ascend/brain/selfcheck.py：SELFCHECK OK（含 3plp 四段；既有
  3slph 相位分账、3krh/3krhm/3krds 判死自付族、3y 审计并表全量通过；既有
  pop_race_audit 精确相等夹具不受影响——空影子账不附分账键）。
- py -3 -B -m unittest sts2-ascend.tests.test_character_strategy
  sts2-ascend.tests.test_review_decision_chain：67 tests OK。
- git diff --check 通过；完整 diff 已回读：brain/policy.py（+48/-1，影子账+
  切分+注记）、brain/knowledge.py（+13，静态键+证据注释）、brain/agent.py
  （+10，披露段）、brain/selfcheck.py（+63，3plp 四段）；工作区另有 assets/
  长路径删除警告为宿主预置现场，与本批无关。随后本地 commit。

## FOLLOW-UP / ROLLBACK

- 未来 3~10 局统计：战斗记录 grep RACE_AUDIT_POST_LATCH_PAY_OBS——若阵亡局
  锁后自付中位 ≥8 且占全场可行动段自损 ≥50%（对照本批 1064:20/28=71%、
  1051:26/50 同型），证实 doomed window 自付集中，下一批行为化（锁窗内
  KRH 压价升档或边际硬顶）；若锁后自付 ≈0（DOM_SCALE 压价门已拦住，行为化
  成效直接显形）或翻盘获胜局锁后自付同样高（自付=翻盘燃料），假设证伪，
  维持纯观测不行为化。
- 继续调整条件：锁后自付高但集中在入锁当回合（口径上界含锁前同回合自付）
  时，先把口径细化到「入锁次回合起」再裁决，不直接加行为。
- 撤回条件：审计段解析报错、竞速判决或门带行为出现任何偏移（本改动零评分
  路径，理论上不可能；一旦出现即 `race_audit_post_latch_pay_obs=False`
  一键撤回，记账与披露同步消失，旧行为零差异）。
