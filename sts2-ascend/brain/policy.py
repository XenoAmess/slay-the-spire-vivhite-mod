...
class Policy:
    ...
    def _score_play(self, card, enemies, incoming, my_block, round_no, pol,
        ...
        if dmg > 0:
            ...
            # 首验窗口 5 条「减员成本加分」注记 100% 与辅助体教义同标——
            # 「去掉减员分后该目标是否仍中标」从留痕不可分辨，杠杆有无独立
            # 行为效应不可计数。同循环复算去分口径（减员分剔除、被减员休眠的
            # 粘性复活），循环收束后对中标者追加翻案/随附段；纯观测不改分，
            # removal_cost_bonus_max=0 时 _rem_cost 恒 0、对账段同灭。
            _winner_rem = 0.0
