...
# 3o) 商店删牌语义：关键词缺失时由握手标志兜底，且必须删最无价值牌而非最高价值牌
    ...
    # 3br-act-trunc（BOSS_ACT_TRUNC，第295~305局批复盘）：Boss 分幕子账本的
    #     int(old)+1 计数器在逐局 ×0.9965 衰减后截断小数，encounters/hp_pool_n
    #     钉死个位数而 float 分子正常累积——分幕「场均战损」虚高 7~9 倍
    #     （vivhite 实测 act1≈427/act2≈530 vs 全量账 61.3；299/303 局前夜留痕
    #     「悲观战损697=场均465×1.5」「869=场均579×1.5」），前夜翻转带恒触发、
    #     安全区锻造沦死代码。修复=写侧浮点累积+一次性作废不可恢复分母的子账本，
    #     读取端按既有 <3 样本设计回落全量账。
    ...
    # ② 一次性迁移作废被污染子账本：分幕均值 633 虚高 → 回落全量账 60
    ...
    assert _bn_post == 30 and abs(_bl_post - 60.0) < 1e-9, \
        f"迁移后未回落全量账（前506批冷启动安全路径）: {_bl_post}/{_bn_post}"
    ...
    assert rp2_know.stats["enemies"]["RP2_BOSS"].get("boss_act"), \
        "标记预置时迁移仍改动既有子账本"
    # ④ 迁移后新样本正确累积并恢复分幕口径：3 场 act1 Boss 战 → 场均 40
    for _i in range(3):
