...
# 3o) 商店删牌语义：关键词缺失时由握手标志兜底，且必须删最无价值牌而非最高价值牌
    ...
    # 3rap) 入锁投影快照披露（第857~872局批复盘异步追及 RACE_PROJ_OBS）：
    #      ① 端到端实测入锁后 _race_audit 携 proj_ttk/proj_tsurv 快照，
    #         后续 tick 投影漂移不改写首次入锁依据；
    #      ② 战斗记录结局后追加「（投影击杀X/存活Y，RACE_PROJ_OBS）」；
    #      ③ race_audit_proj_obs=False 严格回落旧格式（回滚锚）；
    #      ④ 快照缺失的旧账/夹具时段落严格回落旧格式（兼容锚）。
    rap_ctx = type("RAPCTX", (), {"combat": None, "current_combat_is_hard": False,
                                  "credit_tags": []})()
    rap_ctx.combat = {"comp_id": "RAP_COMP", "node_type": "Monster"}

    def rap_state(turn_no, hp_now, incoming, deck, t3_attack=False):
        if turn_no < 3:
            hand = [
                {"index": 0, "card_id": "RAP_HIT", "name": "速攻", "playable": True,
                 "energy_cost": 1, "requires_target": True, "valid_target_indices": [0],
                 "dynamic_values": [{"name": "Damage", "current_value": 10}]},
                {"index": 1, "card_id": "RAP_HIT2", "name": "速攻二号", "playable": True,
                 "energy_cost": 1, "requires_target": True, "valid_target_indices": [0],
                 "dynamic_values": [{"name": "Damage", "current_value": 10}]}]
        else:
            hand = [{"index": 0, "card_id": "RAP_SHLD", "name": "壁垒甲", "playable": True,
                     "requires_target": False,
                     "dynamic_values": [{"name": "Block", "current_value": 8}]}]
        return {
            "screen": "COMBAT", "available_actions": ["play_card", "end_turn"],
            ...
            "combat": {"player": {"current_hp": hp_now, "max_hp": 80, "block": 0, "energy": 3},
                       "hand": hand,
                       "enemies": [{"index": 0, "enemy_id": "RAP_COMP", "name": "滚雪球伏地虫",
                                    "current_hp": 55, "max_hp": 60, "block": 0,
                                    "is_alive": True, "is_hittable": True,
                                    "intents": [{"total_damage": incoming}]}]},
            "run": {"current_hp": hp_now, "max_hp": 80, "gold": 0, "floor": 5, "deck": deck}}

    rap_deck = [{"card_id": f"RAP_S_{i}", "card_type": "Skill", "energy_cost": 1,
                 "dynamic_values": [{"name": "Block", "current_value": 8}]} for i in range(3)]
    rap_pol = policy.Policy(knowledge.Knowledge(Path(tempfile.mkdtemp(prefix="sts2-selfcheck-rap-"))),
                            random.Random(11))
    for _t, _inc in ((1, 4), (2, 7), (3, 24)):
        _d = rap_pol.decide(rap_state(_t, 71, _inc, rap_deck), rap_ctx)
        rap_ctx.credit_tags.extend(_d.tags)
    _rap_a = rap_pol._race_audit
    assert _rap_a.get("latched") and _rap_a.get("latch_round") == 3, \
        f"RACE_PROJ_OBS 夹具未按预期入锁: {_rap_a}"
    assert (isinstance(_rap_a.get("proj_ttk"), int)
            and isinstance(_rap_a.get("proj_tsurv"), int)
            and _rap_a["proj_ttk"] > _rap_a["proj_tsurv"] > 0), \
        f"入锁时未快照投影击杀/存活回合: {_rap_a}"
    _rap_snap = (_rap_a["proj_ttk"], _rap_a["proj_tsurv"])
    _d4 = rap_pol.decide(rap_state(4, 71, 30, rap_deck), rap_ctx)
    rap_ctx.credit_tags.extend(_d4.tags)
    assert (rap_pol._race_audit.get("proj_ttk"), rap_pol._race_audit.get("proj_tsurv")) == _rap_snap, \
        f"首次入锁后的投影快照被后续 tick 改写: {rap_pol._race_audit}"

    # ② ③ ④ agent 战斗记录披露段
    rap_agent = agent.Agent(dict(agent.DEFAULT_CONFIG))
    rap_agent.ctx.reset_for("RAP_RUN", 0)

    def _rap_agg2():
        return {"comp_id": "RAP_COMP2", "floor": 17, "node_type": "Boss",
                "hp_lost_sum": 40.0, "rounds": 7, "won": False, "died": True,
                "hp_start_pct": 0.8, "open": False, "from_event": False,
                "obs_hp_pool": 250.0, "obs_fire_sum": 40.0, "obs_fire_rounds": 3}

    rap_agent.policy._race_audit = {"latched": True, "latch_round": 2, "esc": False,
                                    "proj_ttk": 13, "proj_tsurv": 5}
    rap_agent.ctx.combat_agg = _rap_agg2()
    rap_agent._flush_combat_agg()
    _rap_note = rap_agent.ctx.combat_notes[-1]
    assert "｜竞速审计：T2判死→实战7回合阵亡" in _rap_note \
        and "（投影击杀13/存活5，RACE_PROJ_OBS）" in _rap_note, \
        f"入锁投影快照未披露进战斗记录: {_rap_note}"
    # ③ 观测键关闭严格回落旧格式
    rap_agent.know.policy["race_audit_proj_obs"] = False
    rap_agent.policy._race_audit = {"latched": True, "latch_round": 2, "esc": False,
                                    "proj_ttk": 13, "proj_tsurv": 5}
    rap_agent.ctx.combat_agg = _rap_agg2()
    rap_agent._flush_combat_agg()
    _rap_note_rb = rap_agent.ctx.combat_notes[-1]
    assert "｜竞速审计：T2判死→实战7回合阵亡" in _rap_note_rb \
        and "RACE_PROJ_OBS" not in _rap_note_rb, \
        f"race_audit_proj_obs=False 未严格回落旧格式: {_rap_note_rb}"
    rap_agent.know.policy["race_audit_proj_obs"] = True
    # ④ 快照缺失（旧账/夹具）时段落自动省略，旧格式逐字不变
    rap_agent.policy._race_audit = {"latched": True, "latch_round": 2, "esc": False}
    rap_agent.ctx.combat_agg = _rap_agg2()
    rap_agent._flush_combat_agg()
    _rap_note_old = rap_agent.ctx.combat_notes[-1]
    assert "｜竞速审计：T2判死→实战7回合阵亡" in _rap_note_old \
        and "RACE_PROJ_OBS" not in _rap_note_old, \
        f"快照缺失的旧账未回落旧格式: {_rap_note_old}"
