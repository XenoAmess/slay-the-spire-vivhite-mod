...
# 3o) 商店删牌语义：关键词缺失时由握手标志兜底，且必须删最无价值牌而非最高价值牌
    ...
    #      d8a11849 落地、宿主事务回滚后 1367~1372 批重实现）：
    #      首验窗口 5 条+本批 7 条注记 100% 与教义/粘性同标，杠杆独立
    #      效应不可计数——新鲜 Policy（无粘性记忆）下 ① 情形去分后神官
    #      （10.4>信徒 9.5）中标故记「翻案」；随附夹具（信徒意图 20 →
    #      去分后信徒 14.0>神官 10.4 本就中标）记「随附」；键=0 时对账段
    #      与加分同灭。
    ...
    vknow_any.policy["hp_gate_stall_esc_early"] = 0  # 钉住原闩锁时点：本夹具单独验证全拦截基础口径，提前放行由 3pese 验证（974~979 批投影口径下交替 0/10 意图的升级账投影已达阈值，属 3pese 族职责）
    ...
    # ④ 浅递增投影（第 974~979 局批复盘修正）：生产真实轨迹的中位增速 ≈3~4/回合
    #    （927~973 放行时点 esc 中位 12~13 ÷ 4 空过回合），上批夹具①用 7/回合陡增
    #    掩盖了旧「前一回合已达阈值」口径的结构不可达——本批 974~979 首个可观测
    #    窗口 ESC_OBS 74 次全落原口径、EARLY 留痕 0 次。浅轨迹 7/11/15/19（趋势
    #    0/4/4/4）：第 3 空过回合后 esc=8<12，旧口径第 4 个 decide 仍拦截；投影
    #    口径 esc+本回合趋势=12≥12 → 第 4 个 decide 提前放行并带 EARLY 留痕。
    vknow_es = _vivhite_know("sts2-selfcheck-vhgate-escshallow-")
    vknow_es.policy["vivhite_hp_cost_play_margin"] = 50.0
    vknow_es.policy["vivhite_hp_gate_stall_turns"] = 6
    vknow_es.policy["vivhite_hp_gate_free_turn_relief"] = 0.0
    vpol_es = policy.Policy(vknow_es, random.Random(11))
    vctx_es = _vgate_ctx()
    for _turn, _inc in ((1, 7), (2, 11), (3, 15)):
        d_es = vpol_es.decide(_vgate_stall(_turn, _inc), vctx_es)
        assert d_es.action == "end_turn", \
            f"浅递增序列达投影阈值前照旧拦截: turn={_turn} {d_es}"
    assert vpol_es._hp_gate_stall_esc == 8, \
        f"浅递增序列 3 空过回合后升级账应为 8: {vpol_es._hp_gate_stall_esc}"
    d_es = vpol_es.decide(_vgate_stall(4, 19), vctx_es)
    assert d_es.action == "play_card", \
        f"投影升级账达阈值且零进展≥3 时闩锁应提前一回合放行: {d_es}"
    assert "VIVHITE_HP_GATE_STALL_ANY" in d_es.reason \
        and "HP_GATE_STALL_ESC_EARLY" in d_es.reason, \
        f"浅递增投影提前放行缺 EARLY 分流留痕: {d_es.reason}"
    assert vpol_es._hp_gate_stall_early_fired \
        and vpol_es._hp_gate_stall_any_fired, \
        "浅递增投影提前放行后闩锁与分流标记必须置位"
    # ⑤ 平稳轨迹零差异对照：趋势 0 的序列投影不增值，闩锁时点维持原口径、
    #    无 EARLY 留痕（投影修正不得改变无升级战斗）
    vknow_ez = _vivhite_know("sts2-selfcheck-vhgate-escflat-")
    vknow_ez.policy["vivhite_hp_cost_play_margin"] = 50.0
    vknow_ez.policy["vivhite_hp_gate_stall_turns"] = 6
    vknow_ez.policy["vivhite_hp_gate_free_turn_relief"] = 0.0
    vpol_ez = policy.Policy(vknow_ez, random.Random(11))
    vctx_ez = _vgate_ctx()
    for _turn, _inc in ((1, 9), (2, 9), (3, 9), (4, 9)):
        d_ez = vpol_ez.decide(_vgate_stall(_turn, _inc), vctx_ez)
        assert d_ez.action == "end_turn", \
            f"平稳序列闩锁时点不得提前: turn={_turn} {d_ez}"
        assert "HP_GATE_STALL_ESC_EARLY" not in d_ez.reason, \
            f"平稳序列不得出现 EARLY 留痕: turn={_turn} {d_ez.reason}"
    d_ez = vpol_ez.decide(_vgate_stall(5, 9), vctx_ez)
    assert d_ez.action == "play_card", \
        f"平稳序列连续拦截≥4 且零进展≥3 后照旧放行: {d_ez}"
    assert not vpol_ez._hp_gate_stall_early_fired, \
        "平稳序列放行不得置位提前放行分流标记"
    ...
    # 3krsc) 判死压价滑溜烧墙豁免（KILL_RACE_HOPELESS_HP_PAY_SLIPPERY_CREDIT，
    #      第 1111~1138 局批复盘）：判死 tick 的 KRH 压价对烧墙期低价謦欬攻击
    #      定价失真——滑溜层把每次命中压成 1 点移除，破层却是解锁后续全额伤害
    #      的必经进度（386~391 批 PAYBACK_SLIPPERY_CREDIT 同一把尺），KRH 门
    #      无破层口径：1138 局 F17 VANTOM（滑溜8层开局）T7~T10 门拦【终止条件】
    #      实付2血（KRH_MARGIN），滑溜3层滞留3回合，T3判死→T13 阵亡；本批
    #      1123/1124/1131/1132/1138 共 7 局 23 处「门拦付血×滑溜层」同帧，5 局
    #      死于 F17。「实付≤逐段命中数+预计破层×键」的攻击不计判死压价分量；
    #      高价单发维持全量压价；键=0 一键回滚（旧行为零差异）。
    assert float(knowledge.DEFAULT_POLICY[
        "kill_race_hopeless_hp_pay_slippery_credit"]) == 1.0, \
        "DEFAULT_POLICY 缺少 kill_race_hopeless_hp_pay_slippery_credit 静态键" \
        "或默认值被改"

    def _krsc_state(turn_no, hp_now, hand, incoming=22, layers=8):
        _st = _krh_state(turn_no, hp_now, hand, incoming)
        _st["combat"]["enemies"][0].update({
            "enemy_id": "VANTOM", "name": "墨影幻灵",
            "current_hp": 170, "max_hp": 173})
        if layers:
            _st["combat"]["enemies"][0]["powers"] = [{
                "power_id": "SLIPPERY_POWER", "name": "滑溜",
                "amount": layers, "is_debuff": False}]
        return _st

    def _krsc_drive(pol_obj, ctx_obj, hand_fn, incoming=22, layers=8):
        for _turn, _hp in ((1, 65), (2, 55)):
            _d = pol_obj.decide(
                _krsc_state(_turn, _hp, hand_fn(), incoming, layers), ctx_obj)
            ctx_obj.credit_tags.extend(_d.tags)
        return pol_obj.decide(
            _krsc_state(3, 45, hand_fn(), incoming, layers), ctx_obj)

    # ① 判死+滑溜8层+压价50：实付2的1命中烧墙攻击豁免判死压价分量——同一
    #    驱动从门拦空过翻转为出牌，决策链带破层豁免留痕
    vknow_sc = _vivhite_know("sts2-selfcheck-krhslip-on-")
    vknow_sc.policy["vivhite_hp_cost_play_margin"] = 1.0
    vknow_sc.policy["kill_race_hopeless_hp_pay_margin"] = 50.0
    vpol_sc = policy.Policy(vknow_sc, random.Random(11))
    d_sc = _krsc_drive(vpol_sc, _krh_ctx(), _krh_hand_vivhite)
    assert d_sc.action == "play_card", \
        f"烧墙期低价謦欬攻击豁免判死压价后应恢复出牌: " \
        f"{d_sc.action}（{d_sc.reason}）"
    assert "判死压价破层豁免" in d_sc.reason \
        and "KILL_RACE_HOPELESS_HP_PAY_SLIPPERY_CREDIT" in d_sc.reason, \
        f"破层豁免缺决策链留痕: {d_sc.reason}"
    # ② 键=0 一键回滚：同一驱动恢复门拦空过旧行为，压价留痕显形、豁免全灭
    vknow_sc0 = _vivhite_know("sts2-selfcheck-krhslip-off-")
    vknow_sc0.policy["vivhite_hp_cost_play_margin"] = 1.0
    vknow_sc0.policy["kill_race_hopeless_hp_pay_margin"] = 50.0
    vknow_sc0.policy["kill_race_hopeless_hp_pay_slippery_credit"] = 0.0
    vpol_sc0 = policy.Policy(vknow_sc0, random.Random(11))
    d_sc0 = _krsc_drive(vpol_sc0, _krh_ctx(), _krh_hand_vivhite)
    assert d_sc0.action == "end_turn", \
        f"豁免键=0 回滚后必须恢复判死压价门拦旧行为: {d_sc0.action}"
    assert "KILL_RACE_HOPELESS_HP_PAY_MARGIN" in d_sc0.reason \
        and "KILL_RACE_HOPELESS_HP_PAY_SLIPPERY_CREDIT" not in d_sc0.reason, \
        f"豁免键=0 回滚后留痕口径异常: {d_sc0.reason}"
    # ③ 高价单发不豁免：实付4>逐段命中1+破层1，362 局自杀螺旋拦截边界不变
    def _krsc_hand_pay4():
        return [{
            "index": 0, "card_id": "VIVHITE_CARD_RECURRENT_STARLIGHT",
            "name": "递推星芒", "card_type": "Attack", "playable": True,
            "energy_cost": 1, "requires_target": True,
            "valid_target_indices": [0],
            "dynamic_values": [{"name": "Damage", "current_value": 13}],
        }]
    vknow_sc4 = _vivhite_know("sts2-selfcheck-krhslip-pay4-")
    vknow_sc4.policy["vivhite_hp_cost_play_margin"] = 1.0
    vknow_sc4.policy["kill_race_hopeless_hp_pay_margin"] = 50.0
    vpol_sc4 = policy.Policy(vknow_sc4, random.Random(11))
    d_sc4 = _krsc_drive(vpol_sc4, _krh_ctx(), _krsc_hand_pay4)
    assert d_sc4.action == "end_turn" \
        and "謦欬出牌门拦下" in d_sc4.reason, \
        f"实付4高价单发不得被破层豁免放行: {d_sc4.action}（{d_sc4.reason}）"
    assert "KILL_RACE_HOPELESS_HP_PAY_SLIPPERY_CREDIT" not in d_sc4.reason, \
        f"实付4高价单发不得出现破层豁免留痕: {d_sc4.reason}"
    # ④ 无滑溜对照：同额压价下同一候选维持门拦（豁免严格限定烧墙期）
    vknow_scn = _vivhite_know("sts2-selfcheck-krhslip-noslip-")
    vknow_scn.policy["vivhite_hp_cost_play_margin"] = 1.0
    vknow_scn.policy["kill_race_hopeless_hp_pay_margin"] = 50.0
    vpol_scn = policy.Policy(vknow_scn, random.Random(11))
    d_scn = _krsc_drive(vpol_scn, _krh_ctx(), _krh_hand_vivhite, layers=0)
    assert d_scn.action == "end_turn" \
        and "KILL_RACE_HOPELESS_HP_PAY_MARGIN" in d_scn.reason \
        and "KILL_RACE_HOPELESS_HP_PAY_SLIPPERY_CREDIT" not in d_scn.reason, \
        f"无滑溜敌人时判死压价不得被豁免: {d_scn.action}（{d_scn.reason}）"
    ...
    #      轮询 tick 仍以相同回合号观测 HP 下降，被记入同回合自损主账。
    #      第1270~1274局批行为化（SELF_LOSS_MAIN_OWN_ONLY）：预注册条件「无自付
    #      牌对局 Y≈N ≥3 独立对局」已达成（1270/1271/1272/1274 全场可行动段≤2；
    #      5 局 25 场合计可行动段 29 vs 非行动段 ≈576）——主账只采可行动段
    #      差值，非行动段桶转为披露被排除的敌方伤害量；回滚键
    #      self_loss_main_own_only=False 严格恢复旧混合口径。
    ...
    # 行为化后主账=可行动段口径：只采 T1 可行动 tick 自付 8，敌方行动阶段 20 排除
    assert slph_pol._race_same_round_loss == 8.0, \
        f"自损主账未切换可行动段口径: {slph_pol._race_same_round_loss}"
    ...
    slph_know_rb0.policy["self_loss_main_own_only"] = False
    slph_pol_rb0 = policy.Policy(slph_know_rb0, random.Random(7))
    slph_ctx_rb0 = _slph_ctx()
    slph_pol_rb0.decide(_slph_state(1, 61, ["play_card", "end_turn"]), slph_ctx_rb0)
    slph_pol_rb0.decide(_slph_state(1, 53, ["play_card", "end_turn"]), slph_ctx_rb0)
    slph_pol_rb0.decide(_slph_state(1, 33, []), slph_ctx_rb0)
    assert slph_pol_rb0._race_same_round_loss == 28.0, \
        "self_loss_main_own_only=False 未严格恢复旧混合口径"
    assert slph_pol_rb0.combat_self_hp_loss_phases() == (8.0, 20.0), \
        "主账口径键不应影响相位分账"
    # 回滚锚①：观测键关闭后分账停止；主账仍按可行动段口径（两键语义正交）
    ...
    assert slph_pol_rb._race_same_round_loss == 8.0, \
        "观测键关闭后主账应仍按可行动段口径（8）"
    assert slph_pol_rb.combat_self_hp_loss_phases() == (0.0, 0.0), \
        "回滚键关闭后仍在分账"
    # 战斗记录披露：可 grep 相位段；行为化后自损N=可行动段X，
    # 非行动段Y=被排除出主账的敌方行动阶段掉血；回滚锚②：观测键关闭严格回落旧口径
    ...
    slph_ag.policy._race_same_round_loss = 8.0
    ...
    assert "｜自损8（可行动段8/非行动段20，SELF_LOSS_PHASE_OBS）" in slph_note, \
    ...
    slph_ag_rb.policy._race_same_round_loss = 8.0
    ...
    _kr_down_ctx = SimpleNamespace(
        died_to_event=None,
        died_in_combat={"comp_id": "BR_BOSS_D", "node_type": "Boss", "rounds": 8,
                        "floor": 17, "hp_lost": 78.0, "stall": False},
        death_was_elite=False, death_hp_pct_at_entry=0.98, credit_tags=[],
        rests_healed_at_full=0, ascension=0, combat_notes=[])
    # a) 换向下调减半：上一步 +0.03（上局释放），本局竞速败北 → −0.015 并留痕
    bsd1 = _kr_damp_knowledge(0.63, 0.03)
    _bsd1_lesson = reflect.finalize_run(bsd1, _kr_down_ctx, victory=False, final_floor=17)
    assert abs(bsd1.policy["kill_race_prior_eff"] - 0.615) < 1e-9, \
        f"换向下调未减半: {bsd1.policy['kill_race_prior_eff']}"
    assert abs(bsd1.policy["kill_race_prior_eff_last_step"] + 0.015) < 1e-9, \
        f"换向下调净步长未落盘: {bsd1.policy['kill_race_prior_eff_last_step']}"
    assert "换向阻尼" in _bsd1_lesson, f"换向阻尼未留痕: {_bsd1_lesson}"
    # b) 连续换向几何收敛：last=−0.015 → 反向释放步长 |last|/2=0.0075
    bsd2 = _kr_damp_knowledge(0.60, -0.015)
    reflect.finalize_run(bsd2, SimpleNamespace(
        died_to_event=None,
        died_in_combat={"comp_id": "BR_ELITE_E", "node_type": "Elite", "rounds": 6,
                        "floor": 31, "hp_lost": 52.0, "stall": False},
        death_was_elite=True, death_hp_pct_at_entry=0.4, credit_tags=[],
        rests_healed_at_full=0, ascension=0, combat_notes=[]),
        victory=False, final_floor=31)
    assert abs(bsd2.policy["kill_race_prior_eff"] - 0.6075) < 1e-9, \
        f"连续换向未几何收敛: {bsd2.policy['kill_race_prior_eff']}"
    assert abs(bsd2.policy["kill_race_prior_eff_last_step"] - 0.0075) < 1e-9, \
        f"收敛步长未落盘: {bsd2.policy['kill_race_prior_eff_last_step']}"
    # c) 同向连击恢复全步长：last=−0.03，本局再次竞速败北 → 全速 −0.03
    bsd3 = _kr_damp_knowledge(0.63, -0.03)
    reflect.finalize_run(bsd3, _kr_down_ctx, victory=False, final_floor=17)
    assert abs(bsd3.policy["kill_race_prior_eff"] - 0.60) < 1e-9, \
        f"同向连击未恢复全步长: {bsd3.policy['kill_race_prior_eff']}"
    # d) 阻尼关闭：kill_race_osc_damp=false 时换向仍全步长（旧版行为）
    bsd4 = _kr_damp_knowledge(0.63, 0.03, damp=False)
