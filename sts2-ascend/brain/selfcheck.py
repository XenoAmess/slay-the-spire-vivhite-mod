...
# 3o) 商店删牌语义：关键词缺失时由握手标志兜底，且必须删最无价值牌而非最高价值牌
    ...
    # 3slr) 自付功能分账观测位（SELF_LOSS_ROLE_OBS，第702~722局批复盘）：
    #      722-F5（JP0GN0UDHX7M，缩小甲虫）逐条链——可行动段自付26中格挡付
    #      （闭域映射×6+闭域投影，≈14~16）与攻击付（终止条件×2+递推星芒，≈12）
    #      接近1:1；相位分账（SELF_LOSS_PHASE_OBS）只拆相位不拆功能，而
    #      「放血僵局型格挡付」与「全攻提速型攻击付」需要相反的行为化
    #      （加快放行 vs 收紧门带），先取3~10局全场分账定方向。夹具：已回执
    #      combat_play_commit 逐牌查目录血税拆桶；同一 ledger 二次同步不重复
    #      计账（游标幂等）；增量回执按游标续账；观测键关闭三桶恒 0（回滚锚①）；
    #      非白绮角色目录外牌恒 0（零差异锚）；战斗记录披露与回滚锚②。
    slr_know = _vivhite_know("sts2-selfcheck-slr-")
    slr_pol = policy.Policy(slr_know, random.Random(7))
    slr_ctx = SimpleNamespace(credit_tags=[
        ("combat_play_commit", "VIVHITE_CARD_CLOSED_DOMAIN_MAPPING",
         False, False, 0.0, 1, ""),
        ("combat_play_commit", "VIVHITE_CARD_TERMINATION_CONDITION",
         False, False, 16.0, 1, ""),
        ("combat_play_commit", "VIVHITE_CARD_VIVHITE_TRANSFORMATION",
         False, False, 0.0, 1, ""),
        ("combat_play_commit", "STRIKE_IRONCLAD", False, False, 6.0, 1, ""),
    ])
    slr_pol._sync_combat_play_successes(slr_ctx)
    assert slr_pol.combat_self_pay_roles() == (2.0, 4.0, 4.0), \
        f"功能分账拆桶错误: {slr_pol.combat_self_pay_roles()}"
    slr_pol._sync_combat_play_successes(slr_ctx)
    assert slr_pol.combat_self_pay_roles() == (2.0, 4.0, 4.0), \
        "同一 credit ledger 二次同步重复计账"
    slr_ctx.credit_tags.append(
        ("combat_play_commit", "VIVHITE_CARD_LUMINOUS_PROJECTION",
         False, False, 10.0, 2, ""))
    slr_pol._sync_combat_play_successes(slr_ctx)
    assert slr_pol.combat_self_pay_roles() == (2.0, 6.0, 4.0), \
        f"增量回执未按游标续账: {slr_pol.combat_self_pay_roles()}"
    # 回滚锚①：观测键关闭三桶停记
    slr_know_rb = _vivhite_know("sts2-selfcheck-slr-rb-")
    slr_know_rb.policy["self_loss_role_obs"] = False
    slr_pol_rb = policy.Policy(slr_know_rb, random.Random(7))
    slr_ctx_rb = SimpleNamespace(credit_tags=[
        ("combat_play_commit", "VIVHITE_CARD_CLOSED_DOMAIN_MAPPING",
         False, False, 0.0, 1, ""),
    ])
    slr_pol_rb._sync_combat_play_successes(slr_ctx_rb)
    assert slr_pol_rb.combat_self_pay_roles() == (0.0, 0.0, 0.0), \
        "回滚键关闭后仍在分账"
    assert knowledge.DEFAULT_POLICY["self_loss_role_obs"] is True, \
        "自付功能分账默认键缺失或未开启"
    # 零差异锚：非白绮角色目录外牌三桶恒 0
    slr_know_ic = knowledge.Knowledge(
        Path(tempfile.mkdtemp(prefix="sts2-selfcheck-slr-ic-")))
    slr_pol_ic = policy.Policy(slr_know_ic, random.Random(7))
    slr_ctx_ic = SimpleNamespace(credit_tags=[
        ("combat_play_commit", "STRIKE_IRONCLAD", False, False, 6.0, 1, ""),
    ])
    slr_pol_ic._sync_combat_play_successes(slr_ctx_ic)
    assert slr_pol_ic.combat_self_pay_roles() == (0.0, 0.0, 0.0), \
        "非白绮角色功能分账应恒 0"
    # 战斗记录披露：可 grep 功能分账段；回滚锚②：观测键关闭严格不出现
    slr_ag = agent_mod.Agent(dict(agent_mod.DEFAULT_CONFIG))
    slr_ag._start_combat({"max_hp": 91, "floor": 5}, "SHRINKER_BEETLE", "Monster", 43)
    slr_ag.ctx.combat["rounds"] = 8
    slr_ag.policy._race_same_round_loss = 26.0
    slr_ag.policy._race_same_round_loss_own = 26.0
    slr_ag.policy._race_same_round_loss_enemy = 4.0
    slr_ag.policy._self_pay_blk = 14.0
    slr_ag.policy._self_pay_atk = 12.0
    slr_ag._settle_combat(21, won=True, died=False)
    slr_ag._flush_combat_agg()
    slr_note = slr_ag.ctx.combat_notes[-1]
    assert "（格挡付14/攻击付12，SELF_LOSS_ROLE_OBS）" in slr_note, \
        f"战斗记录缺功能分账披露: {slr_note}"
    slr_ag_rb = agent_mod.Agent(dict(agent_mod.DEFAULT_CONFIG))
    slr_ag_rb.know.policy["self_loss_role_obs"] = False
    slr_ag_rb._start_combat({"max_hp": 91, "floor": 5}, "SHRINKER_BEETLE", "Monster", 43)
    slr_ag_rb.ctx.combat["rounds"] = 8
    slr_ag_rb.policy._race_same_round_loss = 26.0
    slr_ag_rb.policy._race_same_round_loss_own = 26.0
    slr_ag_rb.policy._race_same_round_loss_enemy = 4.0
    slr_ag_rb.policy._self_pay_blk = 14.0
    slr_ag_rb.policy._self_pay_atk = 12.0
    slr_ag_rb._settle_combat(21, won=True, died=False)
    slr_ag_rb._flush_combat_agg()
    slr_note_rb = slr_ag_rb.ctx.combat_notes[-1]
    assert "｜自损26" in slr_note_rb and "SELF_LOSS_ROLE_OBS" not in slr_note_rb, \
        f"回滚键关闭后战斗记录未回落旧口径: {slr_note_rb}"
