...
# 3o) 商店删牌语义：关键词缺失时由握手标志兜底，且必须删最无价值牌而非最高价值牌
    ...
    # 3kdh) 知识恶魔 Ponder 回血净口径（KNOWLEDGE_DEMON_PONDER_HEAL，
    #      第697~701局批复盘）：原生 PonderMove 每 4 回合回 30 血（v0.111.0
    #      mechanics/monsters.jsonl：CreatureCmd.Heal 30×players + 力量+2；
    #      行动循环 CURSE→SLAP→OVERWHELMING→PONDER），竞速投影 ttk=pool/dpt
    #      对回血失明——701 局（V8D6QSFBP3MV）F33 意图序列
    #      0/17/24/11/0/19/30/13/0/21 实证 T4/T8 两次 Ponder 实回 60 血，
    #      等效血池 439 vs 字面 379，ttk 系统性低估、判死偏晚（T10 阵亡）。
    #      夹具：血 30、意图 EMA≈4 → tsurv≈8；池 200、实测 dpt 17×1.35≈23
    #      → 裸 ttk≈9 贴线不判死（margin 后 9.5）；净口径 23-7.5=15 →
    #      ttk≈13>9.5 判死翻转。① 知识恶魔在场：净口径留痕且 ttk9→13、竞速判死显形；
    #      ② 键=0：严格回滚裸口径（无注记、不判死）；③ 同参数非知识恶魔
    #      敌人：无注记不判死（机制隔离，零误伤）。
    kdh_ctx = type("KDHCTX", (), {"combat": {"comp_id": "KNOWLEDGE_DEMON",
                                             "node_type": "Boss"},
                                  "current_combat_is_hard": False,
                                  "credit_tags": []})()

    def kdh_state(turn_no, enemy_id, enemy_name):
        hand = [
            {"index": 0, "card_id": "KDH_HIT", "name": "速攻", "playable": True,
             "energy_cost": 1, "requires_target": True,
             "valid_target_indices": [0],
             "dynamic_values": [{"name": "Damage", "current_value": 10}]},
            {"index": 1, "card_id": "KDH_HIT2", "name": "速攻二号",
             "playable": True, "energy_cost": 1, "requires_target": True,
             "valid_target_indices": [0],
             "dynamic_values": [{"name": "Damage", "current_value": 10}]}]
        return {
            "screen": "COMBAT", "available_actions": ["play_card", "end_turn"],
            "turn": turn_no,
            "combat": {"player": {"current_hp": 30, "max_hp": 80, "block": 0,
                                  "energy": 3},
                       "hand": hand,
                       "enemies": [{"index": 0, "enemy_id": enemy_id,
                                    "name": enemy_name, "current_hp": 200,
                                    "max_hp": 379, "block": 0, "is_alive": True,
                                    "is_hittable": True, "powers": [],
                                    "intents": [{"total_damage": 4}]}]},
            "run": {"current_hp": 30, "max_hp": 80, "gold": 0, "floor": 33,
                    "deck": [{"card_id": f"KDH_D_{i}", "card_type": "Attack",
                              "energy_cost": 1,
                              "dynamic_values": [{"name": "Damage",
                                                  "current_value": 3}]}
                             for i in range(4)]}}

    def kdh_policy(enemy_id="KNOWLEDGE_DEMON", enemy_name="知识恶魔"):
        p = policy.Policy(knowledge.Knowledge(
            Path(tempfile.mkdtemp(prefix="sts2-selfcheck-kdh-"))),
            random.Random(11))
        accepted_combat(p, kdh_state(1, enemy_id, enemy_name), kdh_ctx)
        # 实测口径就位：预置两回合持续输出 40；accepted_combat 已记一回合
        # 出牌样本，合成实测 dpt≈17，换挡上浮 ×1.35 → ≈23
        p._krace_turns = 2
        p._krace_dmg = p._krace_dmg_sustained = 40.0
        return p

    # ① 知识恶魔在场：净口径 ttk 9→13 翻转判死，注记显形
    pol_kdh1 = kdh_policy()
    d_kdh1 = pol_kdh1.decide(kdh_state(2, "KNOWLEDGE_DEMON", "知识恶魔"),
                             kdh_ctx)
    assert ("KNOWLEDGE_DEMON_PONDER_HEAL" in d_kdh1.reason
            and "ttk9→13" in d_kdh1.reason
            and "斩杀竞速投影" in d_kdh1.reason), \
        f"知识恶魔 Ponder 回血未入净口径/未翻转判死: {d_kdh1.action}（{d_kdh1.reason}）"
    # ② 键=0：严格回滚裸口径（无注记、不判死，零差异）
    pol_kdh2 = kdh_policy()
    pol_kdh2.know.policy["knowledge_demon_ponder_heal_rate"] = 0
    d_kdh2 = pol_kdh2.decide(kdh_state(2, "KNOWLEDGE_DEMON", "知识恶魔"),
                             kdh_ctx)
    assert ("KNOWLEDGE_DEMON_PONDER_HEAL" not in d_kdh2.reason
            and "斩杀竞速投影" not in d_kdh2.reason), \
        f"knowledge_demon_ponder_heal_rate=0 未回滚裸口径: {d_kdh2.action}（{d_kdh2.reason}）"
    # ③ 同参数非知识恶魔敌人：无注记不判死（机制隔离）
    pol_kdh3 = kdh_policy(enemy_id="KDH_OTHER", enemy_name="其他魔")
    d_kdh3 = pol_kdh3.decide(kdh_state(2, "KDH_OTHER", "其他魔"), kdh_ctx)
    assert ("KNOWLEDGE_DEMON_PONDER_HEAL" not in d_kdh3.reason
            and "斩杀竞速投影" not in d_kdh3.reason), \
        f"非知识恶魔敌人误伤净口径: {d_kdh3.action}（{d_kdh3.reason}）"
