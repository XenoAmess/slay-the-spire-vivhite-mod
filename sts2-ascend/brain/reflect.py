...
BOUNDS = {    "elite_min_hp_pct": (0.35, 0.9),
    ...
    # 重置 60.0 恢复旧行为零差异）；下限 15.0（第 553~559 批复盘由 30.0 扩档：
    # 559 局 30.0 触底封账「三级旋钮全尽、謦欬证据彻底停止吸收」，而 559 局仍
    # 拿 19 张生命支付牌、F33 Boss 自损27/掉血110 阵亡、555 局 F17 Boss
    # 自损39/掉血30，证据方向未变）。15.0 仍低于起始卡组血税 20，防软顶归零
    # 使首张謦欬牌即被计价、锁死整套机制；回滚=下限恢复 30.0
    "vivhite_life_cost_deck_cap": (15.0, 60.0),
}
            ...
            if len(_life_picks) >= 2:
                ...
                def _lc_tighten(why: str) -> None:
                    ...
                    if _pm_head >= 0.5 - 1e-9:
                        ...
                        # 謦欬卡组阵亡证据改接拿牌端血税软顶
                        # vivhite_life_cost_deck_cap（每级 -5.0，下限 15.0——
                        # 第 553~559 批复盘由 30.0 扩档，559 局 30.0 已触底封账）——
