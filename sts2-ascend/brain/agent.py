...
class Agent:
    ...
    def _flush_combat_agg(self) -> None:
        ...
        if (_ra.get("latched")
            ...
            # 判死时点误差分档（RACE_AUDIT_LATCH_GAP，第790~806局批复盘首落地、
            # 上线前被 runner 启动失败安全撤销（9fcdeb26 反向 9011dcf3），
            # 第807~812局批基于当前 HEAD 重实现同一语义，纯观测）：台账只有
            # 胜负计数，「判死是否系统性过早（全攻换挡在仍可存活的窗口内
            # 自证死期）」每批只能逐局 grep combat note 的「T3判死→实战11
            # 回合」字符串。本批 8 场入锁战斗 died gap 1/3/3/3/5/8 回合、
            # won gap 1/1 回合，分布本身就随 stats digest 进后续 packet：
            # died gap 主体 ≥3 → 判死过早、预注册行为化收紧换挡；主体 ≤1
            # → 判死准时、停止该方向投入。gap=实战终结回合-latch_round，
            # 按 le1/2_3/4_6/ge7 分档并累计 sum/n（均值可读），won/died
            # 分前缀；不参与任何评分/阈值分支，回滚=删除本段。
            try:
