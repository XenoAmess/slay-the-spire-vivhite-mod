...
DEFAULT_POLICY = {
    ...
    "vivhite_hp_gate_slippery_exempt": True,  # 謦欬门滑溜破层豁免（VIVHITE_HP_GATE_SLIPPERY_EXEMPT，
                                      # 第 225~230 局批复盘新增，静态键）：目标带滑溜层时，余量门
                                      # 不再拦「实付 ≤ pay_cap 的伤害牌」——命中只失 1 血时牌的真实
                                      # 价值是破层解锁后续伤害（230 局 F17 VANTOM T1/T2/T5 低价破层
                                      # 攻击被拦、真实伤害迟至 T7 解锁后阵亡）；False 一键回滚旧语义
    "vivhite_hp_gate_slippery_pay_cap": 2.0,  # 滑溜破层豁免的实付血上限（静态键）：只放行低价
                                      # 破层攻击（弦光投影/尺度变换/终止条件实付 1~2 血）；高价謦欬
                                      # （绯红定积分实付 12）破单层照样拦。≤0 即关闭豁免（同回滚）
