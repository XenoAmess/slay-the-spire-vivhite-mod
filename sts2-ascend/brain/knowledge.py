...
DEFAULT_POLICY = {
    ...
    # --- 竞速判死换挡上浮校准（第823~832局批复盘新增，静态键） ---
     ...
     "race_latch_dpt_uplift_eff": 0.20,  # esc 桶换挡上浮接替（RACE_ESC_DPT_UPSHIFT_EFF，
      ...
      "race_audit_proj_obs": True,     # 入锁投影快照披露（RACE_PROJ_OBS，第857~872局批复盘
                                       # 异步追及新增，静态键）：竞速审计战斗记录旧口径只有
                                       # 「T2判死→实战7回合阵亡」，入锁时的投影击杀/存活
                                       # 回合数不入账——本批 15/15 入锁战全负、实战存活超出
                                       # 入锁点 2~12 回合不等，判死生存投影的悲观校准率无从
                                       # 逐场对账。入锁 tick 快照 ttk/tsurv（policy 端
                                       # _race_audit.proj_ttk/proj_tsurv，首次入锁后不改写），
                                       # 战斗记录结局后追加「（投影击杀X/存活Y，RACE_PROJ_OBS）」。
                                       # 纯观测：不参与任何评分/阈值分支；置 False 严格回滚
                                       # 旧版记录格式（零行为差异，selfcheck 对照锚）
