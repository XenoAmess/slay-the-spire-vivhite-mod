...
DEFAULT_POLICY = {
    ...
    "vivhite_hp_cost_play_margin": 0.0,  # 謦欬出牌余量门（VIVHITE_HP_PLAY_MARGIN_GATE，第 165~169 局
     ...
     "vivhite_hp_margin_gate_yield": 1,  # 謦欬门让位（VIVHITE_HP_PLAY_MARGIN_GATE_YIELD，
                                       # 第 170~186 局批复盘新增，静态键）：178~186 九局拦门
                                       # 172 次（104 次拦起手打击）但致命战自损占比未降，
                                       # 满足第 165~169 批预注册证伪——门由绝对否决退化为
                                       # 换序偏好：拦下将导致本回合无牌可出时放行评分最高
                                       # 被拦候选；1=让位（默认），0=旧绝对否决口径（回滚）
