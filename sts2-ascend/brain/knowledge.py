...
DEFAULT_POLICY = {
    ...
    "vivhite_hp_lockout_obs": True,  # 謦欬 1HP 锁死观测位（VIVHITE_HP_LOCKOUT_OBS，
                                  # 第 615~619 局批复盘新增，静态键）：本批 5 局中 4 局
                                  # 终局战出现「全部非诅咒手牌因謦欬会令生命低于1」
                                  # 收口（615-F33、616-F17、618-F45、619-F17 T9/T10，
                                  # 619 局 Boss 战 T8 自付到 1 血后 T9/T10 全手
                                  # blocked_by_hook、意图13回合零格挡阵亡），但此前只在
                                  # 决策 reason 长文本留痕，combat_notes 无计数，复盘
                                  # 无法量化锁死频率与致死关联、也无法判断是否需要
                                  # 「锁死保留线」行为化。开启时战斗记录披露
                                  # 「謦欬锁死N回合（VIVHITE_HP_LOCKOUT_OBS）」；
                                  # 纯观测不改任何评分/动作，置 False 即严格回落旧
                                  # 战斗记录口径（selfcheck 对照锚），非白绮角色零改动
                                  # （锁死分支本身只对白绮卡组触发）
