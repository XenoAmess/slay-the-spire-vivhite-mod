...
DEFAULT_POLICY = {
    ...
    "race_flip_pyrrhic_obs": True,  # 惨胜翻盘影子分账（RACE_FLIP_PYRRHIC_OBS，第1459~1465局批复盘新增，
                                    ...
                                    # 35/80≈44%。惨胜实质佐证判死（该场确为 run-losing），却以「误报」
                                    # 身份计入 won 并驱动 waiver/heal 双闸放宽前夜裁决。本键只追加
                                    # stats.race_audit.won_pyrrhic 计数与战斗记录注记，判决/评分/台账
                                    # 消费口径零改动；False 严格回滚（无计数无注记）
    "race_flip_pyrrhic_hp_frac": 0.4,  # 惨胜判线：单场掉血 ≥ 此比例×最大生命即记惨胜。
                                    # 本批分层证据：两场翻盘 35/37（44%~46% of 80），非翻盘胜利场
                                    # 掉血 0~23（≤29% of 80），0.4 落于两层之间
    # --- Boss 攻坚（第 82~83 批复盘） ---
