...
DEFAULT_POLICY = {
    ...
    "potion_self_harm_reserve_hp": 1,  # 自伤药水使用后的最低保留血（同上批次）：旧版
                                    ...
                                    # 上调把「贴死自残」也纳入
    "potion_fortifier_block_gate": 1,  # 固化药水零甲门（第 489~504 局批复盘新增，静态键）：
                                    # FORTIFIER 固化药水「将你的格挡变为三倍」原生语义为
                                    # GainBlock(当前格挡×2)，0 甲时净收益恒 0；防御/回复
                                    # 分支旧版只按交药线喝不读当前格挡——504 局 F17 Boss
                                    # T6、493 局 F33 T7 均在回合开始 0 甲时首动作倒进
                                    # （格挡每回合清零，回合首喝必为 0），446/451/465 局
                                    # 同理由复现。开启时 0 甲跳过且不计 tried（立甲后本场
                                    # 仍可兑现），使用时理由披露「实兑+N甲
                                    # （FORTIFIER_BLOCK_OBS）」。置 0 = 整体回滚旧行为
    # --- 战斗端补丁键（第 58~59 局复盘） ---
