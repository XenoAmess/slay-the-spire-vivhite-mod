...
DEFAULT_POLICY = {
    ...
    "sleep_guard_pass_obs": True,  # 沉睡放行对账（SLEEP_GUARD_PASS_OBS，第 1425~1429 局批复盘）：
                                   # veto 只留痕拦截侧，放行侧（牌面≤敌甲不唤醒/可击杀）无痕——
                                   # 1429 局 F17 族母 T1 痛击 8 伤打出、T2 意图 0、T3 意图 19，
                                   # 与 1404/1412「T3 沉睡/T4 自然醒」形态不同（提前一回合被唤醒），
                                   # 但存活决策无任何 veto 输入读数可复算该放行是否合法。攻击牌
                                   # 中标且目标沉睡≥sleep_guard_min_stacks 且非击杀时，注记披露
                                   # 牌面与敌甲对照：veto 在产时牌面必≤敌甲；注记若出现牌面>敌甲
    ...
    "enrage_skill_tax": 2.0,  # 激怒技能税（ENRAGE_SKILL_TAX，第637~656局批复盘）：原生
                              # EnragePower.AfterCardPlayed 在玩家打出 Skill 牌时给持有者
                              # +Amount 力量，出牌评分此前零感知——656 局 F48 实验体 #C24
                              #（快照 ENRAGE×2）每回合被打 3~5 张技能牌，意图 20→70 以
                              # ≈+10/回合滚雪球，竞速审计 T8 判死、实战 T11 阵亡；592/630
                              # 局 F48 同 Boss（#C21/#C22）同型阵亡，3 局达线。每张技能牌
                              # 按「激怒层数×本键」扣分：边际 0 费增益/抽牌压到阈值下，
                              # 大额格挡照过；攻击/能力不触发激怒严格零差异；
                              # 0 = 关闭（严格回滚旧口径）
    "vital_spark_skill_tax": 2.0,  # 活力火花技能税（VITAL_SPARK_SKILL_TAX，第657~678局批复盘）：原生
                              # VitalSparkPower 给全部 Skill 牌附 Tainted(Amount)，打出即得
                              # Amount 层污染、本回合受到的每次攻击伤害 +Amount（回合末移除）。
                              # 657 局 F25 / 667 局 F30 / 672 局 F25 / 678 局 F31 四场感染棱柱
                              #（快照 VITAL_SPARK×2 可读）逐 tick 实证：每张技能牌落手下一
                              # tick 意图 +2~4；678-F31 意图 15→51、掉血61。本回合有攻击
    ...
    "barricade_bank_value": True,  # 壁垒存续格挡银行（BARRICADE_BANK_VALUE，第 1420~1424 局批复盘新增，
                                   # 静态键）：溢出贬值的前提「格挡在回合开始时被移除、超出当前意图缺口
                                   # 的部分白费」在壁垒（BARRICADE_POWER）存续时不成立——溢出甲全额存入
                                   # 后续回合。1424 局 F17 瀑布巨兽：T1 已打壁垒，T9 意图 0 剩 1 能量
                                   # 手握防御被「溢出大挡贬值」评 0.0 空过，T10 意图 25 仅 7 甲吃 18
                                   # （21→3），T13 自爆 42 对 17 甲 3 血阵亡；若 T9 存入 7 甲，T10 只吃
                                   # 11。壁垒存续时格挡全额按有用计价（保留缺口外部分的原价，不再跌到
                                   # block_excess_value）；False 一键回滚旧口径（零差异）
    ...
    "power_commit_lowhp_obs_hp_pct": 0.45,  # 低血承诺观测位阈值（第913局批复盘）：
                                  ...
                                  # 0.45 低血线同源；观测留痕随折减继续计数，置 0 即
                                  # 同时关闭观测与折减（折减条件与本阈值同源）
    "power_commit_lowhp_discount": 0.5,  # 低血承诺折减系数（ENGINE_COMMIT_LOWHP_DISCOUNT，
                                  # 第1343~1348局批复盘，913 批观测位按预注册结案行为化）：
                                  # 留痕累计 15 独立对局 16 场（≥3 达线），11/16 当场
                                  # 阵亡、4/16 判死后救回、1/16 非致命——urgent 低血态
                                  # 存活视界付不起引擎复利，开局承诺按本系数折减；
                                  # ≥1.0 严格回滚全额承诺旧口径（注记逐字不变）
    "hand_tax_fire_obs": True,    # 手牌滞留税对账火力观测位（第808~812局批复盘）：
                                  ...
                                  # 留痕开关：置 False 即整体关闭（判决侧入账由
                                  # race_hand_tax_fire 独立控制）
    "race_hand_tax_fire": 1,      # 手牌滞留税入竞速判决（RACE_HAND_TAX_FIRE，
                                  # 第1388~1392局批复盘）：808~812 批预注册「税后
                                  # 火力反事实翻转判决 ≥3 独立对局 → 行为化」达线
                                  # （807-F23 毒素 / 812-F9 感染 / 1392-F17 灵魂异鱼
                                  # BECKON×2=12/回合、联合复核幻影可行后 T8 阵亡）。
                                  # 存活分母与对账火力按 max(火力, 意图+税) 入账
                                  # （EMA 已含历史实付税，max 不重复计价；税不经
                                  # esc 上浮；BOSS_SUSTAIN_NET_HP 续航口径不叠加）。
    ...
    "vivhite_race_self_loss_payback_gate": 1,  # 自付占主导时单体低回报门：仅拦
                                                # 实付血量大于实际移除且未斩杀的
                                                # 白绮攻击；0=关闭并回滚旧竞速豁免。
    "vivhite_race_payback_slippery_credit": 1.0,  # 低回报门滑溜破层抵扣
                                                  # （第386~391局批复盘）：每层实际
                                                  # 破层按本值计入回报抵扣——391局
                                                  # F17 VANTOM T2/T5/T6 弦光投影
                                                  # （实付2血）被「实付2>移除1」压到
                                                  # 禁玩线，竞速态残能空留，意图5→28
                                                  # 滚雪球后 T10 阵亡。放行仅限
                                                  # 实付≤移除+破层抵扣的低价烧墙，
    ...
    "knowledge_demon_curse_tax": 1.0,  # 知识恶魔诅咒四选机制税（第481~488局
                                        ...
                                        # index 0 的瓦解，回执丢失后被冷却轮换
                                        # 静默推向最差诅咒（488 局 F33 选懒惰→
                                        # 次回合 3 牌全 blocked_by_hook，Sloth
                                        # 出牌上限 3）。税值按机制分极：心灵腐化
                                        # -1、瓦解 -2-5×(1-血线)、衰朽 -5、
                                        # 懒惰 -8；0=四诅咒恢复同价旧口径
                                        # （零差异回滚）。
    "ui_option_cooldown_forced_wait": True,  # 强制入组屏冷却等待闸（第590~595局批复盘，
                                        ...
                                        # 抑制时，等待有界冷却（≤4 decide）恢复精确目标，
                                        # 而不是立即强吃严格更差的剩余候选——595 局 F33
                                        # 知识恶魔次屏瓦解被抑制后强吃懒惰（税-8.0 最差），
                                        # SlothPower 锁死末段 5 次出牌致 Boss 余 ~16 血
                                        # 阵亡；488 局 F33 同型。False=回滚旧轮换口径
                                        # （零差异）。
    "self_loss_phase_obs": True,  # 自损账相位分账观测位（第1245~1265局批复盘）：
                                  # 同回合自损主账按采样 tick 的 can_play 影子拆成
                                  # 可行动段/非行动段两桶，战斗记录披露
                                  # 「自损N（可行动段X/非行动段Y，SELF_LOSS_PHASE_OBS）」。
                                  # 1265-F17 实证：全场零自付牌（无御血术打出、VANTOM
                                  # 无反伤）却记「掉血61｜自损53」，53=全部非致命敌方
                                  # 伤害之和——state["turn"] 在敌方行动阶段不前进，
                                  # end_turn 后敌方伤害落进同回合主账。第1270~1274局批
                                  # 主账行为化后非行动段桶=被排除出主账的敌方伤害量；
                                  # 置 False 即停止分账与披露（不影响主账口径）
    "self_loss_main_own_only": True,  # 自损主账可行动段口径（第1270~1274局批复盘
                                  # 行为化）：SELF_LOSS_PHASE_OBS 预注册条件「无自付牌
                                  # 对局 Y≈N ≥3 个独立对局」已达成——本批 5 局 25 场
                                  # 可行动段合计 29 vs 非行动段 ≈576（污染≈95%）；
                                  # 1274 局零生命支付牌卡组场场记「自损」9~34 全为
                                  # 非行动段敌方伤害。主账 _race_same_round_loss 改为
                                  # 只采可行动段（can_play tick）差值：战斗记录
                                  # 「自损N」、died_in_combat.self_hp_loss 与 Vivhite
                                  # 隔离（_self_paid_round 差分）同步恢复真实自付
                                  # 语义。已知口径边界：自付牌结算动画期间 can_play
                                  # 短暂 False 的支付会漏计（有界低估，方向保守）。
                                  # 置 False 即恢复旧混合口径（selfcheck 对照锚）
    ...
    "boss_eve_race_audit_heal_hp_cap": 0.65,  # 竞速审计回血的专用血量上限（第 1236~1240 局批复盘新增）：
                                     # 旧闸复用 boss_eve_smith_hp_pct，该旋钮为「安全区锻造」语义演化
                                     # 0.65→0.45 后把审计纠错带静默压窄（1237 前夜 55%、1238 前夜 64%
                                     # 在台账判死→获胜 47%≥30% 证据下仍被必败弃疗上砧；1238 以 51 血
    ...
    "lethal_race_free_card_exempt": True,  # 0费牌免禁玩（LETHAL_RACE_FREE_CARD_EXEMPT，第1139~1145局批复盘
                                      ...
                                      # False 一键回滚旧禁玩（零差异）
    "lethal_race_free_function_exempt": True,  # 致死竞速0费功能牌保留行动（VIVHITE_LETHAL_FREE_FUNCTION_EXEMPT）：
                                                # 若唯一合法的0费牌能抽牌/回能，长期生命代价估值不得把它
                                                # 压成结束回合；只在白绮 kill_race+本地致死且即时功能底分为正时
                                                # 抬回出牌阈值，付费牌、非致死牌和无功能收益牌不变；False 回滚
    ...
    "vivhite_hp_gate_stall_turns": 6,  # 謦欬门僵局放行（VIVHITE_HP_GATE_STALL_BREAK，第 231~243 局
                                      # 批复盘新增，静态键）：余量门顶格 3.0 后在低危长战制造放血
                                      # 死循环——243 局 F3 史莱姆战拖 56 回合（自损46/掉血39）、238 局
                                      # F2 战 76 回合阵亡（自损64/掉血78）、235 局 F3 战 19 回合阵亡
                                      # （自损46/掉血68）：门每回合拦下 2 血攻击牌，残能救场转而反复
                                      # 付 2 血格挡，战斗永不结束、自损反超敌方伤害。只统计「回合
                                      # 结束仍有謦欬候选被拦、且本回合敌意图已被格挡全覆盖」的连续
                                      # 回合（拦门没换来任何减伤的纯拖延回合）；达到此值后本场停用
                                      # 余量门并闩锁（仅撤附加门槛，普通评分/致死豁免不变），回合
                                      # 理由留进度与放行标记。0=关闭（一键回滚旧行为零差异），
                                      # 非白绮角色零改动
    "vivhite_hp_gate_stall_any_turns": 4,  # 謦欬门全拦截僵局放行（VIVHITE_HP_GATE_STALL_ANY，第 537~552
                                      # 局批复盘新增，静态键）：旧闩锁只认「意图被格挡全覆盖」连续链，
                                      # 意图高低交替、格挡只覆盖低峰的普通战里结构性不可达——549 局
                                      # F2 意图 7/13 交替、格挡 9，低危链 1/6→0/6 循环 17 回合，全程
                                      # 零输出、救场反复付 2 血格挡，自损50/掉血78 阵亡（538 局 F3、
                                      # 546 局 F4 同型；343~354 批观测账 354 局 F3 已实证）。统计
                                      # 「回合结束仍有謦欬候选被拦」的连续回合（不论覆盖）≥此值且敌
                                      # 血量零进展≥3 回合时闩锁放行（仅撤余量门附加门槛，评分/致死
                                      # 豁免不变）。0=关闭（一键回滚，旧行为零差异），非白绮角色零改动
    "vivhite_hp_terminal_lock_obs": 1,  # 生命支付终端锁观测（VIVHITE_HP_TERMINAL_LOCK_OBS，1203~1204局
                                      # 复盘新增，静态键）：1203-F33 与 1204-F48 收口时，服务端已把
                                      # 全部非诅咒手牌标为 blocked_by_hook，白绮仍有 2 能量却只能结束回合；
                                      # 现有逐张 END_TURN 审计缺少稳定事件键，无法按「支付终端锁→随后掉血/
    ...
    "ritual_window_skip_obs": 1,        # 引擎仪式窗口空过观测（VIVHITE_RITUAL_WINDOW_SKIP_OBS，第1243~1275局
                                        ...
                                        # 未上线、自损28阵亡；1262 局 F24/F25 三次同型。仅在「评估后无值得出
    ...
    "vivhite_hp_gate_free_turn_relief": 1.0,  # 謦欬门意图0自由回合减免（VIVHITE_HP_GATE_FREE_TURN_RELIEF，
                                       ...
                                       # 30+ 点输出，竞速审计 T4 判死→实战 7 回合阵亡；生涯 517 局文件
                                       # 332 局共 2239 处「敌意图总伤0+謦欬出牌门拦下」同帧记录，本批
                                       # 505~511 全部 7 局复现。自由回合自付绝无当回合致死可能（原生
                                       # hook 禁自付致死）且 LIVE_ESTIMATE 已计价血税——门带在此类回合
                                       # 是纯输出压制。敌意图总伤≤0 回合余量门分量按 (1-本值) 折算
                                        # （1.0=自由回合撤门带回归普通阈值；复打税分量不减免）；0=关闭
                                        # （一键回滚旧行为零差异），非白绮角色零改动
    "vivhite_hp_gate_free_turn_relief_hard_only": 1,  # 减免硬仗限定（VIVHITE_HP_FREE_TURN_HARD_ONLY，
                                        # 第 512~529 局批复盘新增，静态键）：减免立项证据是精英战
                                        # （511 局 F11），但本批 10/18 局 60+ 处「减免过门」绝大多数
                                        # 落在普通 Monster 战（2W8Z56 局 F12 完美综合色单发实付16血
                                        # 亦靠减免），对应 513-F5 自损26/掉血20、514-F2 自损20/掉血
                                        # 10、529-F6 自损18/掉血6 的普通战放血形态。1=减免仅硬仗
                                        # （Elite/Boss）生效，普通战恢复门带（放血僵局由既有
                                        # STALL_BREAK 闩锁兜底）；0=普通战也减免（回滚至上批行为，
                                        # 零差异），非白绮角色零改动
    "vivhite_hp_gate_uncovered_obs": 1,  # 謦欬门未覆盖拦截观测（VIVHITE_HP_GATE_STALL_UNCOVERED，
                                       # 第 343~354 局批复盘新增，静态键）：354 局 F3 SHRINKER_BEETLE
                                       # 意图 7/13 交替、格挡 9 只覆盖低峰，僵局放行「连续全覆盖」
                                       # 条件结构性不可达（进度止步 2/6 即被高危回合清零），余量门
                                       # 永久锁死謦欬攻击、残能救场反复付 2 血格挡，19 回合自损57/
                                       # 掉血78 放血阵亡。开启时回合收口处加记「有謦欬候选被拦但
                                       # 意图未覆盖」的连续回合数并留痕，不改放行/评分/动作；
                                       # 0=关闭（一键回滚旧行为零差异），非白绮角色零改动
    "hp_gate_stall_esc_obs": 1,        # 謦欬门空过升级账观测（HP_GATE_STALL_ESC_OBS，第 873~899 局
                                       # 批复盘新增，静态键）：门拦空过回合在敌意图升级轨迹下并非
                                       # 免费——本批 369 次门拦空过中 98 次同帧带意图升级注记、
                                       # 累计 +762 意图增量（27/27 局全覆盖），但逐回合「意图升级
                                       # +N」注记与闩锁拦截回合数都没有聚合「空过期间累计升级」，
                                       # STALL_ANY 闩锁（4回合+零进展3）在升级轨迹下是否放得太晚
                                       # 无法裁决。开启时门拦空过留痕与闩锁放行注记披露该累计值；
                                        # 不改放行/评分/动作（纯观测锚）。0=关闭（注记消失，旧行为
                                        # 零差异），非白绮角色零改动
    "hp_gate_stall_esc_early": 1,      # 升级轨迹提前放行（HP_GATE_STALL_ESC_EARLY，第 953~973 局
                                        # 批复盘新增，静态键）：927~973 观测窗结算——21 个闩锁放行
                                        # 时点的「空过期间意图趋势累计」18/21≥10（中位 12~13），闩锁
                                        # 按原阈值放行时敌意图已净涨约 12 点，升级轨迹下每多空过
                                        # 一回合直接把战斗推深（973 局 F22 门拦空过后竞速 T3 判死
                                        # →实战 6 回合阵亡）。1=升级账达阈值时 STALL_ANY 连续拦截
                                        # 要求减 1 回合（下限 2，仅原阈值≥3 启用），敌血量零进展
                                        # ≥3 兜底不动；0=一键回滚（旧行为零差异），非白绮零改动
    "hp_gate_stall_esc_early_intent": 12,  # 提前放行的升级账阈值（同批新增，静态键）：取 927~973
                                        # 放行时点累计的中位下沿；投影升级账（已累计+本回合
                                        # _intent_trend，第 974~979 局批修正——旧「前一回合已达
                                        # 标」口径在中位 ≈3~4/回合增速下结构不可达，首个可观测
                                        # 窗口 EARLY 留痕 0 次）≥此值才允许提前一回合放行
    "hp_gate_stall_any_near_miss_obs": 1,  # 全拦截链近失观测（HP_GATE_STALL_ANY_NEAR_MISS_OBS，
                                        # 第 1104~1110 局批复盘新增，静态键）：1105 局 F4 缩小
                                        # 甲虫 12 回合战「连续拦截2/4」两度重建两度被低于阈值/
                                        # 不可用回合断链清零，STALL_ANY 闩锁全程未放行，9 回合
                                        # 零输出自损31/掉血41 阵亡（竞速 T9 判死→T12 阵亡）——
                                        # 跨回合「重建→近失→再重建」无法从单帧进度注记聚合，
                                        # 闩锁阈值/断链语义是否需重设无法裁决。链 ≥2 未闩锁
                                        # 清零记一次近失并记峰值，全拦截进度留痕旁披露；不改
                                        # 拦截/放行/评分/动作（纯观测锚）。0=关闭（注记消失，
                                        # 旧行为零差异），非白绮角色零改动
    "kill_race_hopeless_hp_pay_obs": 1,  # 竞速判死自付观测（KILL_RACE_HOPELESS_HP_PAY_OBS，
                                       # 第 900~926 局批复盘新增，静态键）：斩杀竞速投影已判
                                       # 「击杀还需 N 回合＞可存活 M 回合」的 tick 仍实付生命
                                       # ——926 局 F48 T8 投影自述击杀还需 4 回合＞可存活 1 回合，
                                       # 仍连打终止条件（实付3）+微分取样+（实付2），6 血付到
                                       # 1 血后阵亡；本批 26/27 局共 239 次判死投影下同帧自付
                                       # 出牌。判死后 39% 翻盘率（race_audit won 488/1249）使
                                       # 自付既可能是翻盘燃料也可能是纯放血，现有留痕无法切片
                                       # 对账。开启时主评分出牌位披露「判死投影下同帧实付血」
                                       # 注记；不改评分/放行/动作（纯观测锚）。0=关闭（注记
                                       # 消失，旧行为零差异）
    ...
    "ringing_single_play_obs": 1,  # 昏眩单卡抉择观测（RINGING_SINGLE_PLAY_OBS，
                                        ...
                                        # 披露本选期望伤害与手牌最高期望伤害备选——1513 局
                                        # F17 仪式兽 T7 单卡选格挡8/抽1（当回合 0 伤害），
                                        # 终局总伤≈237 vs 血池 252，差距恰约一张最高伤
    ...
    "kill_race_hopeless_hp_pay_margin": 1.0,  # 竞速判死自付压价门（KILL_RACE_HOPELESS_HP_PAY_MARGIN，
                                        ...
                                        # 部署时滞后）6/6 局共 93 次/269 血，≈15.5 次/局 ≥9 次/局
                                        # 离线基线且全落阵亡局，预注册行为化路径兑现。斩杀竞速
                                        # 判死 tick（含锁持）謦欬实付在余量门带上追加 实付×本键
                                        # 的压价分量：边际付血被拦（不为必败投影继续放血），超
                                        # 带顶高分打出照旧放行（软压价，判死后翻盘的高分燃料
                                        # 不受压），致死回合豁免与余量门一致，僵局闩锁停用余量
                                        # 门时同步停用。0=一键回滚（旧行为零差异），非白绮角色
                                        # 零改动
    "kill_race_hopeless_hp_pay_dom_cap": 5.0,  # 判死自付压价 DOMINATES 比值放大上限
                                        # （KILL_RACE_HOPELESS_HP_PAY_DOM_SCALE，第 1037~1051 局
                                        # 批复盘新增，静态键；第 1146~1162 局批 3.0→5.0）：判死
                                        # tick 上实测自付速率≥敌方净损
                                        # 速率（VIVHITE_RACE_SELF_LOSS_DOMINATES）时，平坦 ×1.0
                                        # 压价带定价不足——1051 局 F33 知识恶魔战 T4~T7 自付
                                        # 8.7→7.0/回合 ≥ 敌方净损 5.0→2.1/回合（比值 1.72→3.32），
                                        # TSURV_INCLUSIVE 报可存活 3.4→1.0、2.4→0.5 回合，实战
                                        # T7 阵亡（自损50/掉血78=64%）；本批 12/15 局共 167 处
                                        # DOMINATES 留痕，致命 Boss 战自损占掉血 32%~64%，而
                                        # 翻盘胜局（1044/1045）DOMINATES 仅 2 处——比值是
                                        # 「翻盘燃料 vs 纯放血」的区分器。DOMINATES tick 压价
                                        # 倍率按 min(本键, 自付/净损比值) 放大：边际付血在自付
                                        # 主导战被拦，超带顶高分燃料照旧放行（软压价）；致死豁免
                                        # 与闩锁停用语义同 KRH_MARGIN。≤1=关闭（一键回滚平坦
                                        # 压价旧行为零差异），非白绮角色零改动。
                                        # 1146~1162 批截顶证据（预注册跟进「DOMINATES 战仍高频
                                        # 付血且战绩无改善→cap 抬到 4~5」兑现）：17 局全负，
                                        # DOMINATES 留痕 181 处/14 局，其中 41 处比值>3 跨
                                        # 1148/1150/1151/1152/1155/1156/1158/1159 共 8 个独立
                                        # 局（峰值 4.45~inf）被 3.0 截顶，判死自付单局高达
                                        # 90/93/84/75 血；抬 5.0 只影响比值>3 tick，比值≤3
                                        # 行为逐字零差异（selfcheck 3krds⑤ 锚住截顶两侧）
    "kill_race_hopeless_hp_pay_free_relief": 1,  # 判死压价意图0自由回合同步减免
                                        # （KILL_RACE_HOPELESS_HP_PAY_FREE_RELIEF，第 1276~1296 局批
                                        # 复盘新增，静态键）：505~511 批意图0自由回合减免只折余量门
                                        # 分量，927~944 批新增的判死压价分量从未接线减免——硬仗
                                        # （Elite/Boss）判死竞速意图0回合，余量门已撤而判死压价仍拦
                                        # 边际謦欬牌：1296 局 F33 T5（22/84 血、意图0、锁持判死）
                                        # 拦下星图检索（抽牌4/回能、实付2血）空过 0 输出，2 回合后
                                        # 阵亡；1294 局 F21（40 血、意图0、判死锁持）拦下负空间+
                                        # 综合色序空过；生涯 runs 账 42 处「判死压价拦下+意图总伤0」
                                        # 跨 34 个独立局（965~1296），远超 3 局证据线。意图0回合
                                        # 自付绝无当回合致死可能（原生 hook 禁自付致死），判死压价
                                        # 在此类回合与余量门同型——纯输出压制。1=判死压价分量随
                                        # 自由回合减免按 (1-relief) 同步折算（hard_only/僵局闩锁/
                                        # 致死豁免语义不变：普通战减免仍被压回、闩锁停用余量门时
                                        # 压价同步停用、致死回合本就豁免）；0=关闭（判死压价不减免，
                                        # 旧行为零差异），非白绮角色零改动（_krh_margin 恒 0 不进分支）
    "vivhite_hp_lethal_cap_gate_margin": 3.0,  # 謦欬致死回合无实体封顶软顶（VIVHITE_HP_LETHAL_CAP_GATE，
                                        # 第 1017~1036 局批复盘新增，静态键）：謦欬余量门带在致死
                                        # 回合整体豁免（「付血换输出买命/抢斩杀当场兑现」），但
                                        # 中标目标处于无实体封顶窗时输出兑现被压到每 hit≈1——
                                        # 1036 局 F48 TEST_SUBJECT 战 T10~T12 连续 hp-cost=2 攻击
                                        # 以 ≈1.0 封顶伤害打出（ENEMY_INTANGIBLE_CAP_OBS 与
                                        # 「致死竞速抢斩杀」同帧），付血买命兑现率坍塌；
                                        # INTANGIBLE_TTK_OBS 已在 974/979/1017/1023/1026/1032/1036
                                        # 共 7 个独立局显形（≥3 局阈值），不得再只登记待观察。
                                        # 致死回合謦欬攻击候选中标无实体封顶窗（评分侧已折算
                                        # 每 hit=1、非击杀）时，在普通阈值上追加 实付×本键 的
                                        # 软顶门带：边际付血被拦、超带顶高分打出照旧放行（与
                                        # KRH_MARGIN 同构的软压价）；默认 3.0 ≈ 余量门+判死压价
                                        # 在非致死判死 tick 的等效门带量级（≈3.75）的保守下沿。
                                        # 0=一键回滚（致死豁免旧行为零差异），非白绮角色零改动
    "vivhite_hp_zero_pressure_gate": 1,  # 謦欬零压付血闸（VIVHITE_HP_ZERO_PRESSURE_GATE，第 577~589
                                        # 局批复盘新增，静态键）：本批战斗记录反复出现「普通战
                                        # 敌方零威胁回合照付血」——589 局 F22 T4 敌意图总伤0、
                                        # 我方90血仍打出分治法阵+实付4血买抽牌提速（全场掉血0｜
                                        # 自损15）；577-F22 掉血9｜自损19、578-F25 掉血3｜自损14
                                        # 同型。意图≤0 回合提速零收益，而余量门带（3.0 顶格）
                                        # 只拦「阈值~阈值+实付×margin」的边际分，高分謦欬牌全额
                                        # 付血照过；life_cost_weight/余量门/血税软顶三级旋钮已
                                        # 全尽封账。开启时普通 Monster 战意图≤0 的非致死/非
                                        # 竞速回合把 hp_pay>0 且本打不击杀目标的謦欬候选视同
                                        # 门拦（击杀豁免：零压回合击杀买断敌方下一回合出手权）；
                                        # Elite/Boss 不启用（硬仗意图0输出压制已被 505~511 批
                                        # 自由回合减免证伪）；拦下候选照旧退出 marginal/残能
                                        # 救场并喂僵局账，STALL_BREAK/STALL_ANY 闩锁兜底死锁；
                                        # 0=关闭（一键回滚旧行为零差异），非白绮角色零改动
    "vivhite_hp_zp_attack_exempt": 1,  # 零压闸謦欬攻击豁免（VIVHITE_HP_ZP_ATTACK_EXEMPT，第
                                        # 596~601 局批复盘新增，静态键）：零压闸立项证据
                                        #（577~589 批）只覆盖抽牌/提速类非攻击牌（589-F22
                                        # 分治法阵+实付4血买抽牌）；本批 6/6 局实证攻击牌
                                        # 被一并硬拦的反效果——601-F27 SPINY_TOAD 阵亡战 T1
                                        #（意图0=Spikes 蓄力回合、原生机制确认无荆棘）整手
                                        # 謦欬攻击被拦空过，输出挤进 T2 荆棘+爆炸回合（掉血
                                        # 47｜自损15，8 回合阵亡）；596 局 12 处、601 局 16
                                        # 处同型（596-F22 T2 拦 6 张）。脚本循环敌人的意图0
                                        # 回合是白打/抢杀窗，压低敌血=缩短战斗收益非零。1=
                                        # 攻击牌不视同门拦（余量门带计价不变，边际付血仍拦，
    ...
    "thorns_reflect_pricing": 1,     # 荆棘反伤计价与自杀式斩杀闸（THORNS_REFLECT_PRICING，第 784~789
                                        ...
                                        # T2 三刀打进荆棘回合单回合自损 20，T5 我方 8 血打出「可击杀」
                                        # 终止条件（实付 4 血），斩杀命中的 5 点反伤把我方打到 0
                                        # 阵亡——反伤是敌方来源伤害，不受原生自付禁致死 hook 保护，
                                        # 评分侧却把自杀式斩杀记成 +4.20 纯收益、反伤全程零计价。
                                        # 1=单体攻击按目标荆棘层×命中数估算反伤：斩杀即自杀（支付
                                        # 后余血 ≤ 反伤）的候选撤销击杀口径（击杀奖励/击杀豁免失效，
    ...
    "vivhite_hp_gate_rescue_block": 1,  # 门拦净保命格挡救场放行（VIVHITE_HP_GATE_RESCUE_BLOCK，
                                       ...
                                       # 格挡通道自带净保命>0 计价（min(block,gap)−实际謦欬−
                                       # 余裕机会成本），把净正保命格挡一并剔除等于把可抵伤害
                                       # 换成贴脸掉血——355 局 F5 死亡战 T1（门拦闭域映射，
                                       # 可抵3/謦欬2/净保命1，hp33 空过后阵亡）、362 局 F17
                                       # Boss 死亡战 T1（可抵5/净保命3，hp69）、364 局 F2/F8
                                       # 五处（净保命3）三局实证（达 evidence_run_threshold）。
                                       # 开启时仅把 block>0 的门拦牌还给救场手牌（謦欬攻击牌
                                       # 维持排除，反死循环语义不变），打出与否仍由救场通道
                                       # 缺口>0/净保命>0 裁决，留痕供后续批次对账；0=关闭
                                       #（一键回滚旧行为零差异），非白绮角色零改动
    "vivhite_hp_repeat_play_tax": 0.5, # 謦欬同回合复打递增税（VIVHITE_HP_REPEAT_PLAY_TAX，第 427~433
                                       # 局批复盘新增，静态键）：433 局 F30 T2 守恒递归（自我复制
                                       # 引擎牌）六连打，90→30 血单回合自损 60（全场自损 70 vs 敌方
                                       # 掉血 68），F30 出场 22 血、Boss 37% 血入场四回合爆毙——
                                       # 评分循环对本回合已实付血税零记忆，余量门 +30 门带对首打
                                       # 与第六打一视同仁（trace 候选分恒 33.1）。同回合第 N 次
                                       # 打出同名謦欬牌时，余量门带追加 实付×(N-1)×此值 的递增
                                       # 门槛（首打零差异、超带顶高分仍放行，软递增非硬上限）；
                                       # 致死回合豁免与余量门一致，僵局放行闩锁停门时同步停用；
                                       # 0=关闭（一键回滚旧行为零差异），非白绮角色零改动
    "vivhite_margin_pick_bonus": 4.0,  # 謦欬余裕供给稀缺加分（VIVHITE_MARGIN_PICK_SCARCITY，第 244~259 局
                                      # 批复盘新增，静态键）：謦欬是全目录机制（58/61 张 life_calculation_cost
                                      # >0），余裕（Margin）是唯一冲抵实付的供给端；本批 16 局中 6 局终局
                                      # 卡组 0 张无条件余裕源且全部早亡（F2/F4/F9/F11/F17×2），最深两局
                                      # （251/252，F33）余裕源 6/3 张，自损普遍≥敌方掉血。静态估值只给
                                      # margin_gain×1.25≈+3.75，竞争不过又一张謦欬攻击。拿牌端按卡组已有
                                      # 无条件余裕源数线性衰减加分（零源全额、≥cap 归零），0=关闭
                                      # （一键回滚旧行为零差异），非白绮角色零改动
    "vivhite_margin_deck_cap": 3.0,   # 余裕源稀缺加分的卡组供给上限：已有无条件余裕源 ≥此值时加分归零
                                      # （防「为供给囤一手余裕牌」的反向注水），与稀缺衰减分母同键
    "vivhite_life_cost_pick_tax": 2.0, # 謦欬血税密度拿牌扣分（VIVHITE_LIFE_COST_DECK_TAX，第 320~336 局
                                      # 批复盘新增，静态键）：拿牌端对卡组累计生命支付零约束——本批
                                      # 17/17 局终局卡组生命支付牌占比 88~100%（目录血税合计 38~120）、
                                      # 实测自损 ≥ 敌方掉血 50%（六局反超），出牌侧双旋钮已全尽
    ...
    "vivhite_life_cost_deck_cap": 60.0, # 血税密度扣分的软顶：卡组目录生命支付合计低于此值零差异
                                      ...
                                      # 第 402~426 局批复盘起兼任謦欬死亡证据链第三级接替旋钮：
                                      # life_cost_weight 触底且出牌余量门顶格后，reflect 每级
                                      # -5.0 下调此软顶（BOUNDS 15.0~60.0——第 553~559 批复盘由
                                      # 30.0 扩档，559 局 30.0 触底封账后证据仍同向；60.0 即旧锚点可回滚）
     ...
     "kill_race_prior_eff_last_step": 0.0,  # 折算率上一次实际施加的带符号净步长（换向阻尼状态键）：
                                      ...
                                      # 不直接参与任何决策分支
     "kill_race_same_run_damp": True,  # 同局先降后释的释放通道换向判定改以本局净步长为参照
                                      # （KILL_RACE_SAME_RUN_DAMP，第 93~148 局批复盘新增，静态键）：
                                      # 旧版两通道各按上局落盘净额判定，相位使降减半而释全速，
                                      # 本批实测每场 F33 型死亡净 +0.02（0/150 胜里 eff 漂到 0.70+
                                      # 逼近 0.72 上限，竞速败北证据名降实升）；开键后同局后手
                                      # 释通道对先手降通道正确折减，死亡局净额回负。false 即
                                      # 整体回滚为旧版口径（零差异）
    ...
    # --- 竞速判死换挡上浮校准（第823~832局批复盘新增，静态键） ---
     ...
     "race_latch_dpt_uplift_eff": 0.20,  # esc 桶换挡上浮接替（RACE_ESC_DPT_UPSHIFT_EFF，
      ...
      "race_upshift_fresh_turns": 2,   # 入锁新鲜窗抑制换挡上浮（RACE_UPSHIFT_STALE，第1409~1413
                                       ...
                                       # T5~T8 锁后仍逐 tick 叠加×(1+换挡上浮0.20)（实测24~30
                                       # →29~35伤/回合校准），同段实际回合伤害仅 9/16（均值
                                       # 12.5），双重计价系统性高估 dpt。入锁满 N 回合（默认 2）
                                       # 后实测窗已含换挡成果，上浮置零并留痕；首判 tick 与新鲜
                                       # 窗内维持原口径。置 0 严格回滚旧版（锁后持续上浮，零
                                       # 行为差异）
      "lethal_survivable_line": True,  # 致死生还线（LETHAL_SURVIVABLE_LINE，第1414~1419局
                                       # 批复盘新增，静态键）：kill_race 致死回合的「非斩杀攻击
                                       # 让位格挡」豁免以「防守已被证伪」为前提，但前提只证伪
                                       # 长期防守——手牌可负担格挡组合足以把本回合致死缺口补回
                                       # 生还线（合计格挡>缺口-生命）时，买命回合自带 5 张新牌
                                       # 与整管能量，非斩杀攻击恢复让位格挡。1415-F17-T5（25 血
                                       # 对 27 意图，防御在手仍打第 3 张打击，差 2 血阵亡）与
                                       # 1418-F17-T3（14 血对 20 意图，坚毅+防御 12 挡可负担仍
      ...
      "race_allin_lethal_cover_obs": True,
                                        ...
                                        # 以 not race_allin 排除败局竞速（514~517/546/1414~1419
                                        # 三批定案维持全攻），1504-F17-T9 首见「覆盖成立仍全攻
                                        # 阵亡」——3 血对 11 意图，防御5+坚毅7（2 费 12 甲）可
                                        # 覆盖缺口 8，全攻 24 伤后硬吃 11 阵亡；同批 1500/1501/
                                        # 1503 终局覆盖均不成立。单批 1 例 <3 局阈值，不足推翻
                                        # 定案——纯观测带内留痕：race_allin 致死回合可负担格挡
                                        # 组合覆盖缺口-生命时记录缺口/生命/组合，评分判决动作
                                        # 零改动；置 False 观测同灭（旧行为零差异）。
                                        # 第1212~1242局批复盘给覆盖审计加謦欬锁链执行模拟
                                        # （RACE_ALLIN_COVER_EXEC_SIM / RACE_ALLIN_COVER_PHANTOM_OBS）：
                                        # 1223 局 F21 的 3 血组合[9+9+8]三张皆 LifeCost 2，
     ...
     "race_esc_latch_hold": True,    # esc（滚雪球）局实测口径竞速入锁后，不再被静态联合
                                      ...
                                      # 终局战 9 回合内提速/防守交替 7 次后阵亡、281-F14 与
                                      # 292-F24 同型（三独立对局），即第632批迟滞锁要消除的
                                      # 攻防摇摆经联合复核出口复发；race_audit 173/414=41.8%
                                      # （esc 桶 33.3%）连续多批超 30% 预注册线，margin/fire
                                      # inflate/dpt uplift 系数杆已用尽。锁持仅作用于「已入锁
                                      # 且 esc_gate」的翻案分支，未入锁与非滚雪球局出口不变，
                                      # 复核结论以留痕形式保留对账。False 即整体回滚旧版
                                      # 逐 tick 翻案（零行为差异）
    ...
    "respawn_instance_confirm": True,  # 同场重生坐实按敌实例归键（RESPAWN_INSTANCE_CONFIRM）：
                                     ...
                                     # 预测击杀落空」不可区分——1473-F21 异螨/1475-F19 偷窃草蜢/
                                     # 1475-F36 咬人卷轴（均无原生重生机制）「全场均为已证实
                                     # 重生体」注记在产并各喂名册 +1 confirmations；1441~1446
                                     # 批预注册「非白名单物种 confirmations ≥+3 独立对局增量
                                     # 即立项写侧实例键修复」本批达线（1457 瀑布巨兽/1473 异螨/
                                     # 1475 偷窃草蜢/咬人卷轴 ≥4 独立对局）。开键后同一实例两次
                                     # 预测落空仍坐实（利齿之眼同型不变），多实例互证不再发生；
                                     # 坐实后的种级抑制与名册写入口径不变。false 严格回滚种级键
                                     # 旧口径（selfcheck 3yr-inst 回滚锚为对照）
    ...
    "exhaust_fizzle_exempt": 1,   # 消耗空转豁免（第 1378~1382 局批复盘，EXHAUST_FIZZLE_EXEMPT）：
                                  # 消耗目标限定「攻击牌」的痛殴型牌在手牌无其他攻击牌时，原生
                                  # OnPlay 的消耗分支取不到目标直接空转（Thrash：Hand.Where(
                                  # Type==Attack) 得 null 即跳过 Exhaust）——本打零烧牌，消耗螺旋
                                  # 上限（硬拦截）与递增罚分（软压分）的「防烧穿卡组」理由均不成立。
                                  # 实证：1382 局 F17 T6 手牌[防御,痛殴,防御,防御]、能量2、意图0、
    ...
    "support_target_bonus": 8.0,  # 多敌战斗中本回合零伤害意图敌人（治疗/增益/蓄力）的定向转火加分：
                                  ...
                                  # 【更正（1356~1360 批复盘，原生机制核读）】旧注释「神官持续强化
                                  # 信徒」被原生代码证伪：KinPriest.RitualMove 与
                                  # KinFollower.PowerDanceMove 的 PowerCmd.Apply<StrengthPower>
    ...
    "removal_cost_bonus_max": 6.0,  # 多敌战斗减员成本加分上限（第 1356~1360 批复盘，REMOVAL_COST_TARGET）：
                                    ...
                                    # 全钉 190 池神官、58 池信徒零转火；移除廉价攻击源消除的是
                                    # 整条未来意图流。本卡即斩目标已有 kill_bonus 不重复加分；
                                    # 重生召唤物与单敌战斗不适用；0=严格回滚旧口径（加分与留痕同灭）。
                                    # 首验对账（1361~1366 批复盘 d8a11849 落地、宿主事务回滚后
                                    # 1367~1372 批重实现）：中标带本加分时尾部追加「减员成本翻案/
                                    # 随附（REMOVAL_COST_FLIP_AUDIT）」段——去除减员分后火线是否
                                    # 旁落，直接计数杠杆的独立行为效应；纯观测不改分，键=0 时
                                    # 对账段与加分同灭。
    ...
    "focus_drift_obs": True,      # 火线漂移观测注（第 772~783 局批复盘，FOCUS_DRIFT_OBS，纯观测不改分）：
                                  # 783 局 F35 CRUSHER+ROCKET 双自我强化体战逐张火线 0→1→0→0→1
                                  # 横跳、7 回合无一减员阵亡（意图滚到 49）；生涯死因榜
                                  # CRUSHER+ROCKET 32.0 / KNOWLEDGE_DEMON 35.8 同型。定向攻击
                                  # 胜者偏离集火记忆且非击杀时留痕新旧目标名，供 grep 统计漂移
                                  # 频率，作为「强化体互拉拖延减员」行为化的前哨证据；
                                  # False=一键回滚（注记消失，评分零差异）
    "focus_drift_damp": 4.0,     # 换线阻尼（第 813~829 局批复盘，FOCUS_DRIFT_DAMP）：772~783 批预登记的
                                  # 行为化闸门已满足——漂移注记在产 40 局命中 18 个 run 文件 32 次（本批
                                  # 8/17 局 17 次），CRUSHER+ROCKET 同型 F33 Boss 战 817/829 两连阵亡
                                  # （829 火线 碾碎爪→火箭→碾碎爪→火箭，4 回合双敌俱存）。旧集火粘性
                                  # 在自我强化/辅助体教义在场时整体休眠，互拉评分零制衡；本键给集火
                                  # 记忆目标一个教义在场仍生效的阻尼分（默认 4.0=support_target_bonus
                                  # 一半：力量≤3 层的边际互拉不再翻盘，力量≥4 层/完整辅助体教义仍可
    ...
    "focus_drift_lock_step": 2.0,  # 火线翻线锁步长（第 857~872 局批复盘，FOCUS_DRIFT_LOCK）：852~856 批预登记的
                                  # 行为化闸门已满足——本批 FOCUS_DRIFT_FLUSH_OBS 35 次命中 10/16 局（≥3 局
                                  # 阈值）、16 局全负；872 局 F33 CRUSHER+ROCKET T1碾碎爪→T2火箭 非击杀翻线
                                  # 后 4 回合双敌俱存阵亡，FOCUS_DRIFT_DAMP 268 次在产但固定 4.0 对「本场
                                  # 已翻过线」零记忆、翻线成本不递增。有效阻尼=focus_drift_damp+本场实际
                                  # 打出火线的非击杀翻线次数×本键（_combat 收口按打出牌记账：击杀换线不记、
                                  # 上一火线目标已死的被迫换线不记、评分侧静默翻线不记）。步长取 2.0 而非
                                  # 整档 4.0：813~829 批「完整辅助体/力量≥4 层教义（拉力 8.0）仍可合法
                                  # 换线」不变量在首次翻线后保留（阻尼 6.0<8.0），仅边际互拉被按住，三次
