...
DEFAULT_POLICY = {
    ...
    "race_flip_pyrrhic_obs": True,  # 惨胜翻盘影子分账（RACE_FLIP_PYRRHIC_OBS，第1459~1465局批复盘新增，
                                    # 宿主 marker 事务安全撤销后由第1466~1472局批按预注册流程重落地，
                                    # 静态观测键）：race_audit 表观翻盘率（won/latched≈45%）贴 30%~46%
                                    # 带上限，但翻盘样本含「惨胜」——1461-F6 判死T3→获胜掉血37/80≈46%、
                                    # 紧接 F8 以 24 血进场 T3判死→T7 阵亡；1464-F17 判死T3→获胜掉血
                                    # 35/80≈44%；1470-F7 判死T5→获胜掉血44/80=55%、紧接 F8 以 30 血
                                    # 进场 T5判死→5回合阵亡（撤销空窗期第三独立样本）。惨胜实质佐证
                                    # 判死（该场确为 run-losing），却以「误报」身份计入 won 并驱动
                                    # waiver/heal 双闸放宽前夜裁决。本键只追加 stats.race_audit.
                                    # won_pyrrhic 计数与战斗记录注记，判决/评分/台账消费口径零改动；
                                    # False 严格回滚（无计数无注记）
    "race_flip_pyrrhic_hp_frac": 0.4,  # 惨胜判线：单场掉血 ≥ 此比例×最大生命即记惨胜。
                                    # 分层证据：三场翻盘掉血 35/37/44（44%~55% of 80），1466-F15 翻盘
                                    # 掉血30（37.5%）与非翻盘胜利场 0~23（≤29%）同侧，0.4 落于两层之间
    # --- Boss 攻坚（第 82~83 批复盘） ---
