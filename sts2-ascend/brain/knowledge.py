...
DEFAULT_POLICY = {
    ...
    "race_death_pool_obs": True,  # 阵亡残血池观测位（RACE_DEATH_POOL_OBS，第 873~899 局
                                  # 批复盘）：policy 逐 tick 采存活敌人 current_hp 合计，
                                  # 阵亡战斗结算在战斗记录披露「阵亡残血池≈N/池M」并随
                                  # died_in_combat.enemy_hp_left 落账。873~899 批 27 连负
                                  # Boss 战竞速审计判死全部实战兑现（T2/T3 判死→4~13
                                  # 回合阵亡），但记录只留掉血/自损，无法区分「差一口气」
                                  # （残池占比小 → 斩杀端加码）与「结构性竞速不可能」
                                  # （残池占比大 → 判死后停付/路径端更早换战力）。
                                  # 纯观测不改任何评分，置 False 即停止披露（采样照留）
