...
DEFAULT_POLICY = {
    ...
    "elite_forced_entry_obs": True,  # 强制精英进场读数观测位（ELITE_FORCED_ENTRY_OBS，
                                     # 第1525~1537局批复盘新增，纯观测）：唯一候选走廊把路线
                                     # 逼进无可绕行精英时，闸门否决被「无其他候选」覆盖强制
                                     # 进场——本批 3 局 4 例（1526-F14/F23 健康进场、1532-F14
                                     # 灰区 62%、1537-F14 入场 10% 血当场阵亡），此前只能靠
                                     # 复盘人工重建链路。强制进场分支追加「入场血量/选中路径
                                     # 进Boss投影/门内死亡投影」三读数，供后续局机械计数
                                     # 「低血强制进场 × 早层投影乐观」交集频率；达线再立项
                                     # 改早层走廊终端精英定价。置 False 观测即隐，强制进场
                                     # 语义、闸门与判决零改动
    # --- 卡组构建 ---
