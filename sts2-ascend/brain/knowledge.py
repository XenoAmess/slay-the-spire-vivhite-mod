...
DEFAULT_POLICY = {
    ...
    "vivhite_hp_gate_stall_turns": 6,  # 謦欬门僵局放行（VIVHITE_HP_GATE_STALL_BREAK，第 231~243 局
                                      ...
                                      # 非白绮角色零改动
    "vivhite_hp_gate_race_exempt": True,  # 竞速态謦欬门攻击牌豁免（VIVHITE_HP_GATE_RACE_EXEMPT，
                                      # 第 260~270 局批复盘新增，静态键）：斩杀竞速投影宣判
                                      # （kill_race=防守线已被判不可行）后余量门仍拦可负担付血
                                      # 攻击牌——270 局 F17 T4 五牌可出意图 0 整回合零输出、终局
                                      # 「击杀还需1回合＞可存活0」差一刀阵亡；243/246/249/253/255/
                                      # 259/265/269 局同型（IDLE_LEAK_RACE 残能审计与拦门同决策
                                      # 互斥），僵局放行计数在 Boss 高危回合恒被清零（本批最高
                                      # 1/6）结构性不可达。kill_race 下有伤害的謦欬牌免附加门槛
                                      # （非伤害謦欬牌仍拦），False=一键回滚（旧行为零差异），
                                      # 非白绮角色 margin 恒 0 不受影响
