...
DEFAULT_POLICY = {
    ...
    "hp_cost_utility_pricing": 1,  # 耗血功能牌计价（HP_COST_UTILITY_PRICING，第1285~1289局批复盘）：
                                   ...
                                   # 禁玩；0 = 关闭（严格回滚旧口径）
    "energy_convert_race_bonus_max": 6.0,  # 回能转换加分上限（ENERGY_CONVERT_RACE，第1373~1377局批复盘）：
                                    ...
                                    # ≈12伤。按「购入能量可立即兑现的最佳攻击面板×0.5」加分（本键封顶），
                                    # 自付血价照上半价扣减、自付归零直死禁玩不变；非判死语境零改动；
                                    # 0 = 关闭（加分与留痕同灭，严格回滚旧口径）
