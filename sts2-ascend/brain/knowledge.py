...
DEFAULT_POLICY = {
    ...
    "kill_race_hopeless_hp_pay_margin": 1.0,  # 竞速判死自付压价门（KILL_RACE_HOPELESS_HP_PAY_MARGIN，
                                        ...
                                        # 零改动
    "kill_race_hopeless_hp_pay_dom_cap": 3.0,  # 判死自付压价 DOMINATES 比值放大上限
                                        # （KILL_RACE_HOPELESS_HP_PAY_DOM_SCALE，第 1037~1051 局
                                        # 批复盘新增，静态键）：判死 tick 上实测自付速率≥敌方净损
                                        ...
                                        # 压价旧行为零差异），非白绮角色零改动
    "kill_race_hopeless_hp_pay_slippery_credit": 1.0,  # 判死压价滑溜烧墙豁免每层抵扣
                                        # （KILL_RACE_HOPELESS_HP_PAY_SLIPPERY_CREDIT，第 1111~1138
                                        # 局批复盘新增，静态键）：判死 tick 的 KRH 压价分量对烧墙期
                                        # 低价謦欬攻击定价失真——滑溜层把每次命中压成 1 点实际移除，
                                        # 但破层是解锁后续全额伤害的必经进度（386~391 批
                                        # PAYBACK_SLIPPERY_CREDIT 同一把尺），KRH 门却无破层口径：
                                        # 1138 局 F17 VANTOM（滑溜8层开局）T7~T10 门拦【终止条件】
                                        # 实付2血（KRH_MARGIN），滑溜3层滞留 3 回合，竞速审计 T3
                                        # 判死→实战 T13 阵亡；本批 1123/1124/1131/1132/1138 共 7 局
                                        # 23 处「门拦謦欬付血×滑溜层在账」同帧留痕，5 局死于 F17。
                                        # 烧墙期「实付≤逐段命中数+预计破层×本键」的攻击候选不计
                                        # 判死自付压价分量（余量门带/复打税/致死豁免逐字不变）；
                                        # 实付≥4 高价单发维持全量压价。0=一键回滚（旧行为零差异），
                                        # 非白绮角色零改动
