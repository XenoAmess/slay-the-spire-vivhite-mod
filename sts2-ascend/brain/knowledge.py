...
DEFAULT_POLICY = {
    ...
    "boss_eve_race_audit_heal_hp_cap_ext": 0.75,  # 竞速审计回血纠错带的贴线扩展顶（第 392~401 局批复盘新增，
                                     # 静态键）：392 局前夜 65/99=65.7% 以 0.7pp 错过 0.65 带顶被必败弃疗
                                     # 改锻造抢走回血（+29.7→约 95 进场，实战 F35 知识恶魔 11 回合阵亡，
                                     # 掉血 65=自损 36+敌方净伤 29，65 血入场整管打空——回血即生还约 30）；
                                     # 判死台账 238/573=41.5%≥30% 预注册线，本批 23 次判死 16 胜 7 死
                                     # （70%）。生效值=max(原上限, 本键)，只放宽不收紧；0 严格复原旧上限
                                     # （旧行为零差异），地图投影同函数镜像
