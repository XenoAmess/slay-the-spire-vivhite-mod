...
DEFAULT_POLICY = {
    ...
    "race_audit_post_latch_pay_obs": True,  # 锁后自付分账观测位（RACE_AUDIT_POST_LATCH_PAY_OBS，
                                        # 第 1052~1064 局批复盘新增，静态键）：竞速判死入锁后
                                        # 的 doomed window 内白绮仍集中实付生命——1064 局 F33
                                        # CRUSHER+ROCKET 战 T4 入锁后 T5~T7 六次自付合计 20 血，
                                        # 占全场可行动段自损 28 的 71%；本批 1053/1058/1060/
                                        # 1062/1063 同为 T2~T4 入锁、T7~T8 阵亡的 3~6 回合窗口，
                                        # 另有 4 场判死后翻盘获胜——锁后自付是「翻盘燃料 vs 纯
                                        # 放血」的下一层切片，但竞速审计段只有入锁回合与实战
                                        # 结局，无锁后自付聚合口径。开启时 policy 按回合边界记
                                        # 逐回合可行动段自付影子账，战斗记录竞速审计段披露
                                        # 「锁后自付X」（口径：入锁当回合计入锁后，为上界）。
                                        # 纯观测锚：不改评分/放行/动作；False=停止记账与披露
                                        # （旧行为零差异）
