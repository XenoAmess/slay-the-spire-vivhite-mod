...
DEFAULT_POLICY = {
    ...
    "self_loss_role_obs": True,  # 自付功能分账观测位（第702~722局批复盘，
                                 # SELF_LOSS_ROLE_OBS）：按服务端已回执的
                                 # combat_play_commit 逐牌查角色静态目录血税，按
                                 # base_damage/base_block 拆「格挡付/攻击付/功能付」
                                 # 三桶，战斗记录在自损段追加披露。722-F5
                                 # （JP0GN0UDHX7M，缩小甲虫）逐条链：可行动段自付
                                 # 26 中格挡付（闭域映射×6+闭域投影，≈14~16）与
                                 # 攻击付（终止条件×2+递推星芒，≈12）接近 1:1——
                                 # 相位分账（SELF_LOSS_PHASE_OBS）只拆相位不拆功能，
                                 # 而「放血僵局型格挡付」（加快放行/压格挡税）与
                                 # 「全攻提速型攻击付」（收紧门带）要求相反的行为化，
                                 # 先取 3~10 局全场分账定方向。目录外与零血税牌
                                 # 天然零差异（非白绮角色三桶恒 0）。纯观测不改
                                 # 评分/门带/动作；置 False 三桶停记、披露消失
                                 # （旧行为零差异，selfcheck 对照锚）
