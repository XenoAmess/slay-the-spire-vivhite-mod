...
def main() -> None:
    ...
    # Central 70% safe area of the final 1000x760 card crop.
    safe_left = round(1000 * 0.15)
    safe_top = round(760 * 0.15)
    safe_right = round(1000 * 0.85)
    safe_bottom = round(760 * 0.85)
    safe_box = [safe_left, safe_top, safe_right, safe_bottom]

    def contained_in_safe_area(box: list[int] | None) -> bool | None:
        if box is None:
            return None
        return (
            box[0] >= safe_left
            and box[1] >= safe_top
            and box[2] <= safe_right
            and box[3] <= safe_bottom
        )
    ...
    # Read-only warning signal for saturated red-family pixels. This does not
    # alter the source and cannot replace visual review of the face and mouth.
    red_like_count = 0
    red_like_bbox: list[int] | None = None
    red_left = card.width
    red_top = card.height
    red_right = -1
    red_bottom = -1
    ...
    for y in range(card.height):
        for x in range(card.width):
            ...
            if (
                saturation >= 0.35
                and value >= 0.15
                and (hue_degrees <= 25 or hue_degrees >= 335)
            ):
                red_like_count += 1
    ...
    if red_like_count:
        red_like_bbox = [red_left, red_top, red_right + 1, red_bottom + 1]
    ...
    metrics = {
        ...
        "deterministic_card_preview": {
            ...
            "alpha_bbox": card_bboxes,
            "central_70_percent_safe_box": safe_box,
            "contained_in_central_70_percent": {
                name: contained_in_safe_area(box)
                for name, box in card_bboxes.items()
            },
            "saturated_red_family_warning": {
                "definition": "A>=64, HSV saturation>=0.35, value>=0.15, hue<=25 or hue>=335 degrees",
                "pixel_count": red_like_count,
                "bbox": red_like_bbox,
